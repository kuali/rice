...
public class DataDictionaryBuilder {
     ...
     * @throws IllegalStateException if any exceptions occurred during calls to add*Entries constructing the dataDictionary
     */
    ...
    /**
     * Given the name of an XML file, or of a directory containing XML files, adds the entries defined in the XML file or files to
     * the DataDictionary being built. Duplicate class entries (antries using a classname already in use) will result in a
     * DuplicateEntryException being added to the cumulative entryExceptions list. If sourceMustExist is true, a nonexistent source
     * will result in a SourceException being thrown.
     * 
     * @param sourceName XML file or directory containing XML files to be added
     * @param sourceMustExist throw a SourceException if the given source cannot be found
     * @throws IllegalArgumentException if the given sourceName is blank
     * @throws DuplicateEntryException if one of the files being added contains an entry using a classname for which an entrry has
     *         already been defined
     * @throws ParseException if an error occurs processing an entry file
     * @throws SourceException if sourceMustExist is true and the source can't be found
     */
    ...
    /**
     * Given the name of an XML file, or of a directory containing XML files, adds the entries defined in the XML file or files to
     * the DataDictionary being built. Duplicate class entries will override earlier class entries.
     * 
     * @param sourceName XML file or directory containing XML files to be added
     * @param sourceMustExist throw a SourceException if the given source cannot be found
     * @throws IllegalArgumentException if the given sourceName is blank
    ...
    /**
     * Asks the current dataDictionary to perform whatever post-construction cleanup it needs to do before being ready for public
     * consumption.
     * 
     * @throws CompletionException if the dataDictionry runs into problems
     */
     ...
     * 
     * @throws SourceException if the given source does not exist, and sourceMustExist is true
     */
    ...
    /**
     * Parses each XML file on the given list, adding the class entry or entries defined there to the current DataDictionary. If
     * allowOverrides is true, treats duplicate entries (second and subsequent entry using a given classname) as an error;
     * otherwise, the last entry processed using a given classname will replace earlier entries.
     * 
     * @param sourceName name of XML file or directory containing XML files
     * @param sourceMustExist throw a SourceException if the given source cannot be found
     * @param allowOverrides
     * @throws ParseException if an error occurs when processing the given list of xmlFiles
     * @throws DuplicateEntryException if allowOverrides is false, and an entry is defined using a classname for which an entry
     *         already exists
     */
    protected synchronized void addEntries(String sourceName, boolean allowOverrides) {
        ...
        List xmlFiles = listSourceFiles(sourceName);

        dataDictionary.setAllowOverrides(allowOverrides);

        setExceptionsOccurred(true);
        String xmlFileNickname = null;
        Digester digester = buildDigester(digesterRules.get());
        try {
            for (Iterator i = xmlFiles.iterator(); i.hasNext();) {
                File xmlFile = (File) i.next();
                xmlFileNickname = ".../" + xmlFile.getParentFile().getName() + "/" + xmlFile.getName();
                boolean success = false;
                int tryCount = 0;
                // to handle exceptions caused by multi-threaded loading of the DD
                // the Struts ActionServlet classes re-initialzes a static variable
                // that removes the property converters (String -> int, etc...)
                //
                // if something calls org.apache.commons.beanutils.ConvertUtils.deregister() while
                // the digester is parsing a file, a method within ConvertUtilsBean throws an NPE when
                // it tries to convert a string from the XML file into the appropriate type for the
                // cooresponding bean property.  The deregister method immediately puts the missing 
                // converter back, but there are a few milliseconds during which this problem can occur.
                //
                // The loop below attempts to handle this problem by allowing the digester to retry a file once if 
                // an NPE is thrown during digester parsing.
                while ( !success && tryCount < 2 ) {
                    try {
                        tryCount++;
                        digester.setErrorHandler(new XmlErrorHandler(xmlFile.getName()));
                        setCurrentDigester(digester);
                        setCurrentFilename(xmlFile.getCanonicalPath());
        
                        digester.push(dataDictionary);
                        digester.parse(xmlFile);
                        success = true;
                    } catch ( SAXException ex ) {
                        if ( tryCount < 2 && ex.getException() instanceof NullPointerException ) {
                            // suppress
                            LOG.warn( "Digester parsing error probably related to concurrency", ex.getCause() );
                            digester.clear();
                        } else {
                            throw ex;
                        }
            ...
            throw new SourceException("unable to load XML file '" + xmlFileNickname + "'", e);
        }
        catch (DuplicateEntryException e) {
            throw new DuplicateEntryException("duplicate entry parsing XML file '" + xmlFileNickname + "'", e);
        }
            ...
            if (saxCause instanceof DuplicateEntryException) {
                ...
                String newMessage = "unable to parse XML file '" + xmlFileNickname + "': " + saxCause.getMessage();
                ...
                LOG.fatal( "unable to parse XML file '" + xmlFileNickname + "'", saxCause );
                throw new ParseException("unable to parse XML file '" + xmlFileNickname + "'", e);
            }
        }
        finally {
            if (digester != null) {
    ...
    /**
     * @param source XML file, or package containing XML files (which, if a package, must end with the ".xml" extension)
     * @return List of XML Files located using the given sourceName
     * @throws IOException if there's a problem locating the named source
     */
    protected List listSourceFiles(String sourceName) {
        List sourceFiles = new ArrayList();

        ClassLoader loader = Thread.currentThread().getContextClassLoader();
        URL sourceUrl = loader.getResource(sourceName);
        if (sourceUrl == null) {
            throw new SourceException("unable to locate file or directory named '" + sourceName + "'");
        }

        File source = new File(sourceUrl.getFile());
        if (source.isDirectory()) {
            listSources(source, sourceFiles);
        }
        else {
    ...
    protected void listSources(File sourceDir, List sourceFiles) {
        File[] xmlFiles = sourceDir.listFiles(new FilenameFilter() {
            public boolean accept(File dir, String name) {
                return (name != null) && name.endsWith(".xml");
            }
        });
        sourceFiles.addAll(Arrays.asList(xmlFiles));

        File[] subdirs = sourceDir.listFiles(new FileFilter() {
            public boolean accept(File subdir) {
                return (subdir != null) && subdir.isDirectory();
            }
        });
        for (int i = 0; i < subdirs.length; ++i) {
            listSources(subdirs[i], sourceFiles);
        }
    }
    ...
    /**
     * This is a rather ugly hack which expose the Digester being used to parse a given XML file so that error messages generated
     * during parsing can contain file and line number info.
     * <p>
     * If we weren't using an XML file to configure Digester, I'd do this by rewriting all of the rules so that they accepted the
     * Digester instance as a param, which would be considerably less ugly.
     */

    /**
     * @return name of the XML file currently being parsed
     * @throws IllegalStateException if parsing is not in progress
     */
