...
<head>
	...
	<script language="JavaScript" type="text/javascript" src="scripts/dhtml.js"></script>
	...
	<c:choose>
		<c:when test="${lookup}" >
			  <c:if test="${not empty KualiForm.headerNavigationTabs}">
				<link href="css/${KualiForm.navigationCss}" rel="stylesheet" type="text/css" />
			  </c:if>
		...
		<div class="msg-excol">
		  ...
		  <div class="right">
			<c:if test="${showTabButtons != '' && showTabButtons == true}">
			 <div class="excol">
				<html:image property="methodToCall.showAllTabs" src="images/tinybutton-expandall.gif" title="show all panel content" alt="show all panel content" styleClass="tinybutton" onclick="javascript: return expandAllTab(document, tabStatesSize); " />
				<html:image property="methodToCall.hideAllTabs" src="images/tinybutton-collapseall.gif" title="hide all panel content" alt="hide all panel content" styleClass="tinybutton" onclick="javascript: return collapseAllTab(document, tabStatesSize); " />
			  </div>
		   </c:if>     
		  </div>
		</div>
