...
<%@ attribute name="extraTopButtons" required="false" type="java.util.List" description="A List of org.kuali.rice.krad.web.ui.ExtraButton objects to display at the top of the page." %>
...
<html>
  ...
  <head>
    ...
    <c:forEach items="${fn:split(ConfigProperties.css.files, ',')}"	var="cssFile">
      <c:if test="${fn:length(fn:trim(cssFile)) >
        0}">
    ...
    <c:forEach items="${fn:split(ConfigProperties.javascript.files, ',')}"	var="javascriptFile">
      <c:if test="${fn:length(fn:trim(javascriptFile)) >
        0}">
  ...
  <c:choose>
    <c:when test="${lookup}" >
      <body 
          onload="placeFocus(); <c:if test='${KualiForm.class.name == "org.kuali.rice.krad.web.struts.form.LookupForm"}'> <c:out value ="${KualiForm.lookupable.extraOnLoad}" /> </c:if> ">
      ...
      <html:form
        ...
        <c:choose>
          ...
          <c:if test="${showDocumentInfo}">
            ...
            <jsp:useBean id="KualiForm" type="org.kuali.rice.krad.web.struts.form.KualiForm" />

            <c:set 
                var="numberOfHeaderRows"
                value="<%=new Integer((int) java.lang.Math.ceil((double) KualiForm.getDocInfo().size()/KualiForm.getNumColumns()))%>"
            />
              ...
              <c:choose>
                <c:when test="${lookup}" >
                  ...
                  <c:otherwise>
                    <table 
                        class="headerinfo"
                        summary="document header: general information"
                        cellpadding="0"
                        cellspacing="0">
                        ...
                        <c:choose>
                          <c:when test="${headerFieldCount > fieldCounter}">
                            ...
                            <c:choose>
                              ...
                              <c:otherwise>
                                <kul:htmlAttributeHeaderCell 
                                    attributeEntryName="${headerField.ddAttributeEntryName}"
                                    horizontal="true"
                                    scope="row" />
                                <td>
                      ...
                      <c:choose>
                        <c:when test="${empty headerDispatch}">
                          <c:forEach var="headerTab" items="${KualiForm.headerNavigationTabs}" varStatus="status">
                            ...
                            <!-- ${headerTab.headerTabNavigateTo}; ${headerTabActive}; ${currentTab} -->
                            <c:choose>
                            ...
                            <span class="tabright ${currentTab ? 'tabcurrent' : ''}">
                              <krad-struts:submit 
                                  value="${headerTab.headerTabDisplayName}"
                                  property="methodToCall.headerTab.headerDispatch.${headerDispatch}.navigateTo.${headerTab.headerTabNavigateTo}"
              ...
              <div class="excol">
                <c:if test="${!empty extraTopButtons}">
                  <c:forEach items="${extraTopButtons}" var="extraButton">
                    <krad-struts:image 
                        src="${extraButton.extraButtonSource}"
                        styleClass="tinybutton"
                ...
                <c:if test="${showTabButtons != '' && showTabButtons == true}">
                  <krad-struts:submit property="methodToCall.showAllTabs" value="show all" 
                              title="show all panel content" alt="show all panel content" />
                  <krad-struts:submit property="methodToCall.hideAllTabs" value="hide all" 
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            ...
            <c:choose>
              ...
              <c:otherwise>
                ...
                <logic:present name="KualiForm" property="returnToActionList">
                  <html:hidden id="KualiForm" path="returnToActionList" />
                </logic:present>
              </c:otherwise>
            </c:choose>
            ...
            <!-- Hold common state -->
            <!--
                 So that JS expandAllTab / collapseAllTab know the tabStates size. Subtract 1 because currentTabIndex =
                 size + 1.
            -->
            <input type="hidden" name="tabStateSize" value="${KualiForm.currentTabIndex-1}" />
            <html:hidden id="KualiForm" path="dialogMode" />
                
            <c:if test="${transactionalDocument || maintenanceDocument}">
              <html:hidden id="documentWebScope" path="session"/>
              <html:hidden id="formKey" path="formKey" />
              <html:hidden id="docFormKey" path="formKey" />
              <html:hidden id="docNum" path="document.documentNumber" />
            </c:if>
