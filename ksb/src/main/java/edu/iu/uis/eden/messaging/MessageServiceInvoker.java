...
public class MessageServiceInvoker implements Runnable {
    ...
    /**
         * Invokes the AsynchronousCall represented on the methodCall on the service contained in the ServiceInfo object on
	...
	if (Core.getCurrentContextConfig().getStoreAndForward() && !methodCall.isIgnoreStoreAndForward()) {
	    ...
	    QName storeAndForwardName = new QName(serviceName.getNamespaceURI(), serviceName.getLocalPart()
		    + RemotedServiceRegistry.FORWARD_HANDLER_SUFFIX);
		...
		LOG.warn("Could not find store and forward service " + storeAndForwardName
			+ ".  Defaulting to regular messaging.");
	    } else {
    ...
    /**
         * Get the service as a topic. This means we want to contact every service that is a part of this topic. We've
    ...
    /**
         * Used in case the thread that dumped this work into the queue is waiting for the work to be done to continue
         * processing.
         * 
         * @param callback
         */
