...
<xsl:stylesheet version="1.0"
  ...
  <xsl:key name="apptPersKey"
    match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD/PERSONNEL/PERSON"
    use="APPOINTMENT/@APPOINTMENT_TYPE"/>
  
  <xsl:key name="apptPersNameKey"
    match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD/PERSONNEL/PERSON"
    use="concat( APPOINTMENT/@APPOINTMENT_TYPE,' ',NAME )"/>
	
	<xsl:key name="apptPersPeriodKey"
		match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD/PERSONNEL/PERSON"
		use="concat( APPOINTMENT/@APPOINTMENT_TYPE,' ',../../@PERIOD_NUMBER )"/>
  
  <xsl:key name="persApptPeriodNameKey"
    match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD/PERSONNEL/PERSON"
    use="concat( APPOINTMENT/@APPOINTMENT_TYPE,' ',../../@PERIOD_NUMBER,' ',NAME )"/>
	
	<xsl:key name="othProfsPeriodKey"
		match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
		/PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'PROFESSIONAL'] |
		/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
		/PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'BIWEEKLY']"
		use="../../@PERIOD_NUMBER"/>
  ...
  <xsl:key name="senPersNameKey"
    match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
    /PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'ACADEMIC-12'] |
    /PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
    /PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'ACADEMIC-9'] |
    /PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
    /PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'SUMMER']"
    use="NAME"/>
  ...
  <xsl:key name="othOthPersPeriodKey"
    match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
    /PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'HRLY&gt;1000hrs'] |
    /PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
    /PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'HRLY&lt;1000hrs'] |
    /PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
    /PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'STDNT-HRLY'] |
    /PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
    /PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'STDNT-WRKSTDY']"
    use="../../@PERIOD_NUMBER"/>
  ...
  <xsl:template match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD" mode="allTasksSummaries">
    ...
    <xsl:variable name="gradStudents"
    			select="key('apptPersPeriodKey',concat( 'STDNT-ACAD',' ',$periodNum ))
    			[ generate-id(.) = generate-id( key( 'persApptPeriodNameKey', 
    			concat( 'STDNT-ACAD',' ',$periodNum, ' ',NAME ) )[1])]"/>
    ...
    <xsl:variable name="othProfSalaries" select="sum(key('othProfsPeriodKey',$periodNum)
         	/@AGENCY_AMOUNT_SALARY)"/>
    <xsl:variable name="gradStudentSalaries" select="sum(key('apptPersKey','STDNT-ACAD')
  ...
  <xsl:template match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD"
    ...
    <!-- Node sets containing groups of non-senior personnel. -->
  	<xsl:variable name="othProfs"
  		...
  		/PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'PROFESSIONAL' and
  		generate-id(.) = generate-id( key( 'othProfsNameKey',NAME ))] |
  		/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD
  		/PERSONNEL/PERSON[APPOINTMENT/@APPOINTMENT_TYPE = 'BIWEEKLY' and 
  		generate-id(.) = generate-id( key( 'othProfsNameKey',NAME ))]"/>
    <xsl:variable name="gradStudents" select="key('apptPersKey','STDNT-ACAD')
      [generate-id(.) = generate-id( key( 'apptPersNameKey', 
      concat( 'STDNT-ACAD',' ',NAME ) ) )]"/>
    ...
    <xsl:variable name="numPostDocs" select="'*'"/>
    <xsl:variable name="numOthProfs" select="count($othProfs)"/>
    <xsl:variable name="numGradStudents" select="count(key('apptPersKey','STDNT-ACAD')
      [generate-id(.) = generate-id( key( 'apptPersNameKey', 
      concat( 'STDNT-ACAD',' ',NAME ) ) ) ] )"/>
    <xsl:variable name="numUgradStudents" select="'*'"/>
    <xsl:variable name="numSecretarial" select="'*'"/>
    <xsl:variable name="numOtherPers" 
      select="count(key('apptPersKey','HRLY&gt;1000hrs')[generate-id(.) = 
      generate-id( key( 'apptPersNameKey', concat( 'HRLY&gt;1000hrs',' ',NAME ) ) ) ] ) 
      + count(key('apptPersKey','HRLY&lt;1000hrs')[generate-id(.) = 
      generate-id( key( 'apptPersNameKey', concat( 'HRLY&lt;1000hrs',' ',NAME ) ) ) ] ) 
      + count(key('apptPersKey','STDNT-HRLY')[generate-id(.) = 
      generate-id( key( 'apptPersNameKey', concat( 'STDNT-HRLY',' ',NAME ) ) ) ] )
      + count(key('apptPersKey','STDNT-WRKSTDY')[generate-id(.) = 
      generate-id( key( 'apptPersNameKey', concat( 'STDNT-WRKSTDY',' ',NAME ) ) ) ] )"/>
    <xsl:variable name="numParticipants" select="'?'"/>
    
    <!-- These sums of salaries are over all Tasks of the Period. -->
    <xsl:variable name="senPersSalaries" select="sum($senPers/@AGENCY_AMOUNT_SALARY)"/>
    <xsl:variable name="postDocSalaries" select="'*'"/>
    <xsl:variable name="othProfSalaries" select="sum(key('apptPersKey','PROFESSIONAL')
    	/@AGENCY_AMOUNT_SALARY) + sum(key('apptPersKey','BIWEEKLY')/@AGENCY_AMOUNT_SALARY)"/>
    <xsl:variable name="gradStudentSalaries" select="sum(key('apptPersKey','STDNT-ACAD')
      /@AGENCY_AMOUNT_SALARY)"/>
    <xsl:variable name="ugradStudentSalaries" select="'-'"/>
    <xsl:variable name="secretarialSalaries" select="'-'"/>
  ...
  <xsl:template name="main">
    ...
    <fo:table table-layout="fixed">
      ...
      <fo:table-body>
        ...
        <xsl:choose>
          <xsl:when test="$periodNum = 0">
            <xsl:apply-templates select="$senPers[ generate-id(.) = 
              generate-id( key('persNameKey',NAME) ) ]" mode="senPersLine">
  ...
  <xsl:template match="PERSONNEL/PERSON" mode="senPersLine">
    <xsl:param name="periodNum"/>
      ...
      <xsl:when test="$periodNum = 0">
        <xsl:for-each select="$nodeContext">
          <xsl:if test="($contextName = 'othProfs' and 
            generate-id(.) = generate-id( key( 'apptPersNameKey', concat( 'PROFESSIONAL',' ',NAME ) ) ) ) 
            or ($contextName = 'othProfs' and
          	generate-id(.) = generate-id( key( 'apptPersNameKey', concat( 'BIWEEKLY',' ',NAME ) ) ) )
          	or ($contextName = 'gradStudents' and
            generate-id(.) = generate-id( key( 'apptPersNameKey', concat( 'STDNT-ACAD',' ',NAME ) ) ) )
                  ...
                  <xsl:choose>
                    <xsl:when test="$contextName = 'othProfs'">
                      <xsl:value-of select="format-number( sum(key( 'apptPersNameKey', 
                        concat( 'PROFESSIONAL',' ',NAME ) )/@AGENCY_AMOUNT_SALARY) +
                      	sum(key( 'apptPersNameKey', concat( 'BIWEEKLY',' ',NAME ) )
                      	/@AGENCY_AMOUNT_SALARY),'###,###')"/>
                    </xsl:when>
                    <xsl:when test="$contextName = 'gradStudents'">
                      <xsl:value-of select="format-number( sum(key( 'apptPersNameKey',
                        concat( 'STDNT-ACAD',' ',NAME ) )/@AGENCY_AMOUNT_SALARY),'###,###')"/>
                    </xsl:when>
                ...
                <xsl:choose>
                  <xsl:when test="$contextName = 'othProfs'">
                    <xsl:value-of select="format-number( sum(key( 'persApptPeriodNameKey', 
                      concat( 'PROFESSIONAL',' ',$periodNum,' ',NAME ))
                    	/@AGENCY_AMOUNT_SALARY) + sum(key( 'persApptPeriodNameKey', 
                    	concat( 'BIWEEKLY',' ',$periodNum,' ',NAME ))
                    	/@AGENCY_AMOUNT_SALARY),'###,###')"/>
                  </xsl:when>
                  <xsl:when test="$contextName = 'gradStudents'">
                    <xsl:value-of select="format-number( sum(key( 'persApptPeriodNameKey',
                      concat( 'STDNT-ACAD',' ',$periodNum,' ',NAME ) )
                      /@AGENCY_AMOUNT_SALARY),'###,###')"/>
                  </xsl:when>
                  <xsl:when test="$contextName = 'othOthPers'">
