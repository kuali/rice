...
/**
 * Provides various mechanisms for an application to customize the behavior of the Document Lookup functionality for
 ...
 *
 * <ul>
 *     <li>The ability to customize document lookup criteria before it gets submitted.</li>
 *     <li>The ability to customize how document lookup criteria is cleared.</li>
 ...
 * {@code DocumentLookupCriteria} as well.</p>
 *
 * <p>Furthermore, the customization hooks afforded by this interface will only be executed when a valid document type
 * has been specified as input to the document lookup.  This is because the customizer is tied to the document type and
 * the document lookup must therefore be able to resolve a valid document type from the user-supplied criteria in order
...
public interface DocumentSearchCustomizer {
     ...
     * lookup being executed, and the resulting customized criteria is what gets passed to the search implementation.
     * It is also invoked in situtations where the document lookup is performed via the api or via the user interface.
     *
     ...
     * invoked whenever the user of the document lookup user interface clicks the "clear" button.  By default all of
     * the lookup criteria is cleared, but this method allows for certain criteria data to be preserved on a clear if
     * desired.
     *
     * <p>A common use of this feature is to preserve the document type that has been selected when clearing criteria
     * for a customized document lookup.
     *
     * <p>It is guaranteed that the document type name on the given criteria will never be null and will always
     * represent a valid document type.</p>
     *
     ...
     * executed but before results are returned to the caller.  It is executed when a document lookup is executed via
     ...
     * provided in the given list of document lookup results.  It is important to note however that ommision from the
     * returned result values does not filter or remove the result from the lookup results.  Generally speaking, this
