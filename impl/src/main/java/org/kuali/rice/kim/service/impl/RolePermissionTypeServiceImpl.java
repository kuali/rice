 ...
 *
 */
public class RolePermissionTypeServiceImpl extends NamespacePermissionTypeServiceImpl {

	{
		inputRequiredAttributes.add(KimAttributes.ROLE_NAME);
	}
	...
	protected boolean performMatch(AttributeSet inputAttributeSet, AttributeSet storedAttributeSet) {
		...
		if (StringUtils.isEmpty(storedAttributeSet.get(KimAttributes.ROLE_NAME))) {
        	return namespaceMatch;
		}

		return namespaceMatch 
			&& inputAttributeSet.get(KimAttributes.ROLE_NAME).equals(storedAttributeSet.get(KimAttributes.ROLE_NAME));
	}
