...
class StyleRepositoryServiceImplTest {
	...
	private def MockFor daoMock
	private def RiceCacheAdministratorMock cacheMock
	private def StyleRepositoryServiceImpl styleRepositoryService;
	...
	private static final StyleBo styleBo = StyleBo.from(style)
	
			
	
    @Before
    void setupStyleServiceImpl() {
        daoMock = new MockFor(StyleDao)
		cacheMock = new RiceCacheAdministratorMock()
        styleRepositoryService = new StyleRepositoryServiceImpl()
		styleRepositoryService.setCache(cacheMock)
		styleRepositoryService.setStyleDao(daoMock.proxyDelegateInstance())
    }
	
	private void verifyMocks() {
		daoMock.verify(styleRepositoryService.styleDao)
	}
	...
	void testGetStyle_valid() {
		...
		styleRepositoryService.setStyleDao(styleDao)
