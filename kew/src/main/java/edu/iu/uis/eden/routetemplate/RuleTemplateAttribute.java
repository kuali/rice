...
public class RuleTemplateAttribute implements WorkflowPersistable, Comparable {
    ...
    public boolean isWorkflowAttribute() {
    	try {
    		Object attributeObject = getWorkflowAttribute();//GlobalResourceLoader.getResourceLoader().getObject(new ObjectDefinition(getRuleAttribute().getClassName()));
    ...
    public boolean isRuleValidationAttribute() {
    	...
    	/*try {
    		Class attributeClass = SpringServiceLocator.getExtensionService().loadClass(getRuleAttribute().getClassName());
    		if (attributeClass == null) {
    			return false;
    		}
    		return RuleValidationAttribute.class.isAssignableFrom(attributeClass);
    	} catch (Exception e) {
    		throw new RuntimeException("Caught error attempting to load RuleValidationAttribute class: " + getRuleAttribute().getClassName(), e);
    	}*/
    }
