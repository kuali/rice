 ...
 * Connects that ServiceConnector to the appropriate CredentialsSource.
 * <p>
 * ServiceConnector will fail if a CredentialsSource for the Service Definition cannot
 * be located.
 * 
 * @author rkirkend
...
public class ServiceConnectorFactory {
	...
	public static ServiceConnector getServiceConnector(
		...
		final ServiceDefinition serviceDefinition = serviceInfo
				.getServiceDefinition();
		final CredentialsSourceFactory credentialsSourceFactory = (CredentialsSourceFactory) Core.getCurrentContextConfig().getObjects().get(Config.CREDENTIALS_SOURCE_FACTORY);
		final CredentialsSource credentialsSource = credentialsSourceFactory != null ? credentialsSourceFactory
				.getCredentialsForType(serviceDefinition.getCredentialsType())
				: null;
		...
		if (serviceConnector == null) {
			throw new RiceRuntimeException("Don't support service type of "
					+ serviceInfo.getServiceDefinition());
		}
