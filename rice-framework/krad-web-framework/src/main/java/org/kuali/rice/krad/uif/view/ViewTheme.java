 ...
 * entire application.
 *
 * The theme has logic for 'dev' mode versus 'test/prod' mode. This is controlled through the <code>krad.dev.mode</code>
 * configuration variable. In development mode it will source in all the CSS and JS files individually (to
 * allow for easier debugging). In non-development mode it will source in a minified file. The name for the minified
 * file can be specified by setting {@link #getMinFileName()}. If not set, it will be formed by using the
 * {@link #getName()}, {@link #getMinVersionSuffix()}, and min suffix (this is the file name generated by the
 * theme builder). To indicate the min file should not be sourced in regardless of the environment, set the
 * property {@link #isIncludeMinFiles()} to false
 *
...
public class ViewTheme extends UifDictionaryBeanBase implements Serializable {
    ...
    public void configureThemeDefaults() {
        boolean inDevMode = getConfigurationService().getPropertyValueAsBoolean(
        ...
        // names and push to top of css and script file lists
            ...
            if (StringUtils.isBlank(this.minFileName)) {
                ...
                this.minFileName = this.name + "." + this.minVersionSuffix + UifConstants.FileExtensions.MIN;
            }
    ...
    /**
     * When {@link #getMinFileName()} is not set, the min file name will be generated using the theme
     ...
     *
     * <p>
     * When min file name is not set it will be generated by using the theme name, version, and min prefix.
     * This corresponds to the min file names generated by the theme builder
     *
     * For example, with name 'kboot' and version '2.3.0' the min file name will be 'kboot.2.3.0.min.js'
     * </p>
     *
     * @return name of min file
     */
    @BeanTagAttribute(name = "minFileName")
