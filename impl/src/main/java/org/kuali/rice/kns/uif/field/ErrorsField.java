...
/**
 * Field that displays error, warning, and info messages for the keys that are matched.  By default, an ErrorsField
 * will match on id and bindingPath (if this ErrorsField is for an AttributeField), but can be set to match on
 * additionalKeys and nested components keys (of the its parentComponent).
 * 
 * In addition, there are a variety of options which can
 * be toggled to effect the display of these messages during both client and server side validation display.
 * See documentation on each get method for more details on the effect of each option.
 * 
...
public class ErrorsField extends FieldBase {
	...
	public void performFinalize(View view, Object model) {
		...
		if(displayMessages){
			...
			//Checks to see if any errors exist for this field, if they do set errorCount as positive
	 ...
	 * This will also combine error messages per a field if that option is turned on.
	 * If displayFieldLabelWithMessages is turned on, it will also find the label by key passed in.
	 * 
	 * @param view
	 * @param key
	 * @param lists
	 * @return
	 */
	private List<String> getMessages(View view, String key, List<AutoPopulatingList> lists){
	 ...
	 * These are the keys that are used to match errors with their component and display them as part of its
	 * ErrorsField.
	 * 
	 * @return
	 */
	 ...
	 * it will also turn off error display for its parent's nestedComponents' ErrorsFields.
	 * 
	 * @param keyList
	 * @param component
	 */
	 ...
	 * display for client side validation, as it may prevent a user from completing a form.  
	 * To turn off client side validation AND its messaging use the applyClientSide
	 * flag on the Constraint itself.
	 * 
	 * TODO this may be changed to: if this is set on a field it will attempt show client side validation errors
	 * in the closest parent container error container
	 * 
	 * @return
	 */
	...
	/**
	 * If displayInfoMessages is true, info messages will be displayed, otherwise they will not.  Client side
	 * validation has no concept of warning or info messages, so this will not effect client side functionality.
	 * 
	 * @return
	 */
	...
	/**
	 * If displayWarningMessages is true, warning messages will be displayed, otherwise they will not.  Client side
	 * validation has no concept of warning or info messages, so this will not effect client side functionality.
	 * 
	 * @return
	 */
	...
	/**
	 * If true, the errorTitle set on this ErrorsField will be displayed along with the error messages.  Otherwise,
	...
	/**
	 * If true, the warningTitle set on this ErrorsField will be displayed along with the warning messages.  Otherwise,
	 * the title will not be displayed.
	 * 
	...
	/**
	 * If true, the infoTitle set on this ErrorsField will be displayed along with the info messages.  Otherwise,
	 * the title will not be displayed.
	 * 
	...
	/**
	 * If true, the error messages will display the an AttributeField's title alongside the error, warning, and
	 * info messages related to it.  This setting has no effect on messages which do not relate directly to
	 * a single AttributeField.
	 * 
	 ...
	 * Otherwise, no messages for this ErrorsField container will be displayed (including ones set to display).
	...
	/**
	 * If true, this ErrorsField will show messages related to the nested components of its parent component, and
	 * not just those related only to its parent component.
	 * Otherwise, it will be up to the individual components to display their messages, if any, in their ErrorsField.
	...
	/**
	 * If true, when this is set on an ErrorsField whose parentComponent has nested Containers or AttributeFields,
	 * it will allow those fields to also show their ErrorsField messages.  Otherwise, it will turn off the 
	 * the display of those messages.  This can be used to avoid repeating information to the user per field, if
	 * errors are already being displayed at the parent's level.  This flag has no effect if displayNestedMessages
	...
	/**
	 * displayCounts is true if the counts of errors, warning, and info messages within this ErrorsField
	 * should be displayed (includes count of nested messages if displayNestedMessages is true).
	 * 
	 * @return
	 */
	...
	/**
	 * The list of error messages found for the keys that were matched on this ErrorsField
	 * This is generated and cannot be set
	 * 
	 * @return the errors
	 */
	...
	/**
	 * The list of warning messages found for the keys that were matched on this ErrorsField
	 * This is generated and cannot be set
	 * @return the warnings
	 */
	...
	/**
	 * The list of info messages found for the keys that were matched on this ErrorsField
	 * This is generated and cannot be set
	 * 
	 * @return the infos
	 */
	...
	/**
	 * The count of error messages found for the keys that were matched on this ErrorsField
	 * This is generated and cannot be set
	 * 
	...
	/**
	 * The count of warning messages found for the keys that were matched on this ErrorsField
	 * This is generated and cannot be set
	 * 
	...
	/**
	 * The count of info messages found for the keys that were matched on this ErrorsField
	 * This is generated and cannot be set
	 * 
	...
	/**
	 * If this is true, the display of messages is being handled by another container.  The ErrorsField html generated
	 * by the jsp will still be used, but it will be placed in different location within the page than the
	 * default to accommodate an alternate layout.  This flag is used by BoxLayoutManager.
	 * 
	 * This flag only applies to ErrorsFields whose parentComponents are AttributeFields.
	 * 
	...
	/**
	 * If true, displays an icon next to each field that has an error (default KNS look).  Otherwise, this icon
	 * will not be displayed.  Note that any icon set through css for the message containers will still appear
	 * and this only relates to the icon directly to the right of an input field.
	 * 
	...
	/**
	 * If true, highlights the parentComponent's container when it has an error/warning/info.  Otherwise, this highlighting
	 * will not be displayed.  Note that the css can be changed per a type of highlighting, if showing a different
	 * color or no color per type of message is desired.
	 * 
