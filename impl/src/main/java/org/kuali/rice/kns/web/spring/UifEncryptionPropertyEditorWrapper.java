...
public class UifEncryptionPropertyEditorWrapper extends PropertyEditorSupport {
	...
	public String getAsText() {
		try {
			if (propertyEditor != null) {
				return KNSServiceLocator.getEncryptionService().encrypt(propertyEditor.getAsText());
			}
			return KNSServiceLocator.getEncryptionService().encrypt(getValue());
	...
	public void setAsText(String text) throws IllegalArgumentException {
		try {
			String value = KNSServiceLocator.getEncryptionService().decrypt(text);
			if (propertyEditor != null) {
