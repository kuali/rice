...
public class KimCommonUtilsTest extends KIMTestCase {
	...
	public void testResolveKimTypeServiceName() throws Exception {
		...
		assertEquals("When null is passed to resolveKimTypeService, should return default kim type service.", defaultKimTypeServiceQName, KimCommonUtilsInternal.resolveKimTypeServiceName(null));
		assertEquals("When empty string is passed to resolveKimTypeService, should return default kim type service.", defaultKimTypeServiceQName, KimCommonUtilsInternal.resolveKimTypeServiceName(""));
		assertEquals("When blank string is passed to resolveKimTypeService, should return default kim type service.", defaultKimTypeServiceQName, KimCommonUtilsInternal.resolveKimTypeServiceName(" "));
		
		// test resolving a custom kim type service name with no namespace
		
		String serviceName1 = "customKimTypeServiceName_noNamespace";
		
		QName serviceQName1 = KimCommonUtilsInternal.resolveKimTypeServiceName(serviceName1);
		...
		QName serviceQName2 = KimCommonUtilsInternal.resolveKimTypeServiceName(serviceName2);
