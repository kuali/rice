...
public class DocumentAuthorizerBase extends AuthorizerBase {
    ...
    public final boolean canInitiate(String documentTypeName, Person user) {
        ...
        return getIdentityManagementService().isAuthorizedByTemplateName(user.getPrincipalId(), nameSpaceCode,
    ...
    public final boolean canReceiveAdHoc(Document document, Person user, String actionRequestCode) {
        ...
        return isAuthorizedByTemplate(document, KNSConstants.KUALI_RICE_WORKFLOW_NAMESPACE,
    ...
    public final boolean canAddNoteAttachment(Document document, String attachmentTypeCode, Person user) {
        ...
        if (attachmentTypeCode != null) {
            additionalPermissionDetails.put(KimAttributes.ATTACHMENT_TYPE_CODE, attachmentTypeCode);
        }
        return isAuthorizedByTemplate(document, KNSConstants.KNS_NAMESPACE,
    ...
    public final boolean canDeleteNoteAttachment(Document document, String attachmentTypeCode,
            String createdBySelfOnly, Person user) {
        Map<String, String> additionalPermissionDetails = new HashMap<String, String>();
        if (attachmentTypeCode != null) {
            additionalPermissionDetails.put(KimAttributes.ATTACHMENT_TYPE_CODE, attachmentTypeCode);
        }
        additionalPermissionDetails.put(KimAttributes.CREATED_BY_SELF, createdBySelfOnly);
        return isAuthorizedByTemplate(document, KNSConstants.KNS_NAMESPACE,
    ...
    public final boolean canSendAdHocRequests(Document document, String actionRequestCd, Person user) {
        ...
        if (actionRequestCd != null) {
            additionalPermissionDetails.put(KimAttributes.ACTION_REQUEST_CD, actionRequestCd);
        }
        return isAuthorizedByTemplate(document, KNSConstants.KNS_NAMESPACE,
    ...
    public boolean canEditDocumentOverview(Document document, Person user) {
        return isAuthorizedByTemplate(document, KNSConstants.KNS_NAMESPACE,
                KimConstants.PermissionTemplateNames.EDIT_DOCUMENT, user.getPrincipalId())
                && this.isDocumentInitiator(document, user);
    }
    ...
    protected final boolean canSendAnyTypeAdHocRequests(Document document, Person user) {
        if (canSendAdHocRequests(document, KEWConstants.ACTION_REQUEST_FYI_REQ, user)) {
            try {
                DocumentTypeDTO docType = KNSServiceLocator.getWorkflowInfoService().getDocType(
    ...
    protected void addStandardAttributes(Document document, Map<String, String> attributes) {
        ...
        if (wd.stateIsInitiated() || wd.stateIsSaved()) {
            attributes.put(KimAttributes.ROUTE_NODE_NAME, PRE_ROUTING_ROUTE_NAME);
        }
        else {
