...
def processXmlFile(file) {	
	...
	<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	    ...
	    <xsl:template match="node()">
	    	<xsl:if test="not( ( @name = 'label' or @name = 'shortLabel' or @name = 'helpDefinition' or @name = 'summary' or @name = 'description') and local-name(.) = 'property' and substring(../@name, (string-length(../@name) - string-length('Document')) + 1) = 'Document' )">
	    		<xsl:copy>
					<xsl:apply-templates select="@*|node()"/>
				</xsl:copy>
	    	</xsl:if>
