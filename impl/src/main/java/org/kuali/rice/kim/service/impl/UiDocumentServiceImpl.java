...
public class UiDocumentServiceImpl implements UiDocumentService {
    ...
    public void saveEntityPerson(IdentityManagementPersonDocument identityManagementPersonDocument) {
        ...
        if (canModifyEntity(initiatorPrincipalId, identityManagementPersonDocument.getPrincipalId())) {
            inactivatingPrincipal = setupPrincipal(identityManagementPersonDocument, kimEntity,
                    origEntity.getPrincipals());
            setupAffiliation(identityManagementPersonDocument, kimEntity, origEntity.getAffiliations(),
                    origEntity.getEmploymentInformation());
            ...
            for (EntityTypeContactInfoBo type : origEntity.getEntityTypeContactInfos()) {
                ...
                if (type.getEntityTypeCode() != null && StringUtils.equals(type.getEntityTypeCode(),
        ...
        if (creatingNew || canOverrideEntityPrivacyPreferences(getInitiatorPrincipalId(
                identityManagementPersonDocument), identityManagementPersonDocument.getPrincipalId())) {
        ...
        IdentityManagementNotificationService service =
                (IdentityManagementNotificationService) KsbApiServiceLocator.getMessageHelper()
                        .getServiceAsynchronously(new QName("KIM", "kimIdentityManagementNotificationService"));
        ...
        if (inactivatingPrincipal) {
            ...
            KimApiServiceLocator.getRoleService().principalInactivated(
    ...
    public void loadEntityToPersonDoc(IdentityManagementPersonDocument identityManagementPersonDocument,
        ...
        identityManagementPersonDocument.setAffiliations(loadAffiliations(kimEntity.getAffiliations(),
                kimEntity.getEmploymentInformation()));
        identityManagementPersonDocument.setNames(loadNames(identityManagementPersonDocument, principalId,
                kimEntity.getNames(), identityManagementPersonDocument.getPrivacy().isSuppressName()));
        ...
        if (entityType != null) {
            identityManagementPersonDocument.setEmails(loadEmails(identityManagementPersonDocument, principalId,
                    entityType.getEmailAddresses(), identityManagementPersonDocument.getPrivacy().isSuppressEmail()));
            identityManagementPersonDocument.setPhones(loadPhones(identityManagementPersonDocument, principalId,
                    entityType.getPhoneNumbers(), identityManagementPersonDocument.getPrivacy().isSuppressPhone()));
            identityManagementPersonDocument.setAddrs(loadAddresses(identityManagementPersonDocument, principalId,
                    entityType.getAddresses(), identityManagementPersonDocument.getPrivacy().isSuppressAddress()));
        }

        List<Group> groups = getGroupService().getGroups(getGroupService().getDirectGroupIdsForPrincipal(
                identityManagementPersonDocument.getPrincipalId()));
    ...
    public List<DelegateBo> getPersonDelegations(String principalId) {
        ...
        List<DelegateMemberBo> delegationMembers = (List<DelegateMemberBo>) getBusinessObjectService().findMatching(
                DelegateMemberBo.class, criteria);
        List<DelegateBo> delegations = new ArrayList<DelegateBo>();
        ...
        if (ObjectUtils.isNotNull(delegationMembers)) {
            for (DelegateMemberBo delegationMember : delegationMembers) {
                if (!delegationIds.contains(delegationMember.getDelegationId())) {
                    ...
                    delegations.add(getBusinessObjectService().findByPrimaryKey(DelegateBo.class, criteria));
                }
            }
        }
        return delegations;
    ...
    protected void loadDelegationsToPersonDoc(IdentityManagementPersonDocument identityManagementPersonDocument) {
        ...
        List<DelegateBo> origDelegations = getPersonDelegations(identityManagementPersonDocument.getPrincipalId());
        if (ObjectUtils.isNotNull(origDelegations)) {
            for (DelegateBo del : origDelegations) {
                if (del.isActive()) {
                    ...
                    documentDelegation.setMembers(loadDelegationMembers(identityManagementPersonDocument,
                            del.getMembers(), (RoleBo) getMember(KimConstants.KimUIConstants.MEMBER_TYPE_ROLE_CODE,
                            del.getRoleId())));
    ...
    protected List<RoleDocumentDelegationMember> loadDelegationMembers(
            IdentityManagementPersonDocument identityManagementPersonDocument, List<DelegateMemberBo> members,
            RoleBo roleImpl) {
        ...
        if (ObjectUtils.isNotNull(members)) {
            for (DelegateMemberBo member : members) {
                ...
                if (pndMember.isActive()) {
                    ...
                    if (roleMember != null) {
                        pndMember.setRoleMemberName(getMemberName(roleMember.getMemberTypeCode(),
                                roleMember.getMemberId()));
                        pndMember.setRoleMemberNamespaceCode(getMemberNamespaceCode(roleMember.getMemberTypeCode(),
                                roleMember.getMemberId()));
                    }
                    pndMember.setMemberNamespaceCode(getMemberNamespaceCode(member.getTypeCode(),
                            member.getMemberId()));
                    ...
                    pndMember.setQualifiers(loadDelegationMemberQualifiers(identityManagementPersonDocument,
                            pndMember.getAttributesHelper().getDefinitions(), member.getAttributes()));
    ...
    protected List<RoleDocumentDelegationMemberQualifier> loadDelegationMemberQualifiers(
        ...
        List<RoleDocumentDelegationMemberQualifier> pndMemberRoleQualifiers =
                new ArrayList<RoleDocumentDelegationMemberQualifier>();
        ...
        if (origAttributeDefinitions != null) {
            for (String key : origAttributeDefinitions.keySet()) {
                origAttributeId = identityManagementPersonDocument.getKimAttributeDefnId(origAttributeDefinitions.get(
                        key));
                        ...
                        if (StringUtils.equals(memberId, identityManagementPersonDocument.getPrincipalId())) {
                            ...
                            if (ObjectUtils.isNotNull(groupMemberships)) {
                                for (GroupMember groupMember : groupMemberships) {
                                    if (groupMember.isActive(new DateTime(System.currentTimeMillis()))
                                            && StringUtils.equals(groupMember.getMemberId(),
                                            identityManagementPersonDocument.getPrincipalId())
                                            && StringUtils.equals(groupMember.getTypeCode(),
                                            KimGroupMemberTypes.PRINCIPAL_MEMBER_TYPE)) {
                                        ...
                                        if (groupMember.getActiveFromDate() != null) {
                                            docGroup.setActiveFromDate(
                                                    groupMember.getActiveFromDate() == null ? null : new Timestamp(
                                                            groupMember.getActiveFromDate().getMillis()));
                                        }
                                        if (groupMember.getActiveToDate() != null) {
                                            docGroup.setActiveToDate(
                                                    groupMember.getActiveToDate() == null ? null : new Timestamp(
                                                            groupMember.getActiveToDate().getMillis()));
                                        }
        ...
        if (ObjectUtils.isNotNull(roles)) {
            for (RoleBo role : roles) {
                if (!roleIds.contains(role.getId())) {
                    ...
                    docRole.setRolePrncpls(populateDocRolePrncpl(role.getNamespaceCode(), role.getMembers(),
                            identityManagementPersonDocument.getPrincipalId(), getAttributeDefinitionsForRole(
                            docRole)));
    ...
    protected AttributeDefinitionMap getAttributeDefinitionsForRole(PersonDocumentRole role) {
        ...
        try {
            ...
            LOG.warn("Not able to retrieve KimTypeService from remote system for KIM Role Type: " + role
                    .getKimRoleType(), ex);
        }
    ...
    protected List<RoleResponsibilityActionBo> getRoleRspActions(String roleMemberId) {
        ...
        return (List<RoleResponsibilityActionBo>) getBusinessObjectService().findMatching(
                RoleResponsibilityActionBo.class, criteria);
    }

    protected List<KimDocumentRoleMember> populateDocRolePrncpl(String namespaceCode, List<RoleMemberBo> roleMembers,
            String principalId, AttributeDefinitionMap definitions) {
        ...
        if (ObjectUtils.isNotNull(roleMembers)) {
            for (RoleMemberBo rolePrincipal : roleMembers) {
                if (rolePrincipal.isActive(new Timestamp(System.currentTimeMillis()))
                        && Role.PRINCIPAL_MEMBER_TYPE.equals(rolePrincipal.getMemberTypeCode())
    ...
    protected List<KimDocumentRoleQualifier> populateDocRoleQualifier(String namespaceCode,
            List<RoleMemberAttributeDataBo> qualifiers, AttributeDefinitionMap definitions) {
    ...
    protected List<PersonDocumentName> loadNames(IdentityManagementPersonDocument personDoc, String principalId,
            List<EntityName> names, boolean suppressDisplay) {
    ...
    public boolean canModifyEntity(String currentUserPrincipalId, String toModifyPrincipalId) {
        return (StringUtils.isNotBlank(currentUserPrincipalId)
                && StringUtils.isNotBlank(toModifyPrincipalId)
                && currentUserPrincipalId.equals(toModifyPrincipalId)) || getPermissionService().isAuthorized(
                currentUserPrincipalId, KimConstants.NAMESPACE_CODE, KimConstants.PermissionNames.MODIFY_ENTITY, null,
    ...
    public boolean canOverrideEntityPrivacyPreferences(String currentUserPrincipalId, String toModifyPrincipalId) {
        return (StringUtils.isNotBlank(currentUserPrincipalId)
                && StringUtils.isNotBlank(toModifyPrincipalId)
                && currentUserPrincipalId.equals(toModifyPrincipalId)) || getPermissionService().isAuthorized(
                currentUserPrincipalId, KimConstants.NAMESPACE_CODE,
                KimConstants.PermissionNames.OVERRIDE_ENTITY_PRIVACY_PREFERENCES, null, Collections.singletonMap(
                KimConstants.AttributeConstants.PRINCIPAL_ID, currentUserPrincipalId));
    }
    ...
    protected boolean canAssignToRole(IdentityManagementRoleDocument document, String initiatorPrincipalId) {
        ...
        if (!getDocumentHelperService().getDocumentAuthorizer(document).isAuthorizedByTemplate(document,
                KimConstants.NAMESPACE_CODE, KimConstants.PermissionTemplateNames.ASSIGN_ROLE, initiatorPrincipalId,
        ...
        if (ObjectUtils.isNotNull(affiliations)) {
            for (EntityAffiliation affiliation : affiliations) {
                if (affiliation.isActive()) {
                    ...
                    List<PersonDocumentEmploymentInfo> docEmploymentInformations =
                            new ArrayList<PersonDocumentEmploymentInfo>();
                    if (ObjectUtils.isNotNull(empInfos)) {
                        for (EntityEmployment empInfo : empInfos) {
                            if (empInfo.isActive() && StringUtils.equals(docAffiliation.getEntityAffiliationId(),
                                    (empInfo.getEntityAffiliation() != null ? empInfo.getEntityAffiliation().getId() :
                                            null))) {
                                ...
                                docEmpInfo.setEmploymentStatusCode(
                                        empInfo.getEmployeeStatus() != null ? empInfo.getEmployeeStatus().getCode() :
                                                null);
                                docEmpInfo.setEmploymentTypeCode(
                                        empInfo.getEmployeeType() != null ? empInfo.getEmployeeType().getCode() : null);
                                docEmpInfo.setActive(empInfo.isActive());
                                docEmpInfo.setPrimary(empInfo.isPrimary());
                                docEmpInfo.setEntityAffiliationId(empInfo.getEntityAffiliation() != null ?
                                        empInfo.getEntityAffiliation().getId() : null);
    ...
    protected boolean setupPrincipal(IdentityManagementPersonDocument identityManagementPersonDocument,
            EntityBo kimEntity, List<PrincipalBo> origPrincipals) {
        boolean inactivatingPrincipal = false;
        ...
        if (ObjectUtils.isNotNull(origPrincipals)) {
            for (PrincipalBo prncpl : origPrincipals) {
                if (prncpl.getPrincipalId() != null && StringUtils.equals(prncpl.getPrincipalId(),
    ...
    protected void setupPrivacy(IdentityManagementPersonDocument identityManagementPersonDocument, EntityBo kimEntity,
    ...
    protected void setupName(IdentityManagementPersonDocument identityManagementPersonDocument, EntityBo kimEntity,
            List<EntityNameBo> origNames) {
        if (!identityManagementPersonDocument.getPrivacy().isSuppressName() || canOverrideEntityPrivacyPreferences(
                ...
                for (PersonDocumentName name : identityManagementPersonDocument.getNames()) {
                    ...
                    if (name.getEntityNameType() != null) {
                        ...
                        if (StringUtils.isNotEmpty(name.getNameTypeCode())) {
                            entityName.setNameType(EntityNameTypeBo.from(getIdentityService().getNameType(
                                    name.getNameTypeCode())));
                        }
                    }
    ...
    protected void setupAffiliation(IdentityManagementPersonDocument identityManagementPersonDocument,
            EntityBo kimEntity, List<EntityAffiliationBo> origAffiliations, List<EntityEmploymentBo> origEmpInfos) {
            ...
            for (PersonDocumentAffiliation affiliation : identityManagementPersonDocument.getAffiliations()) {
                ...
                if (affiliation.getAffiliationType() != null) {
                    ...
                    if (StringUtils.isNotEmpty(affiliation.getAffiliationTypeCode())) {
                        entityAffiliation.setAffiliationType(EntityAffiliationTypeBo.from(
                                getIdentityService().getAffiliationType(affiliation.getAffiliationTypeCode())));
                    }
                }
                ...
                if (ObjectUtils.isNotNull(origAffiliations)) {
                    ...
                    for (EntityAffiliationBo origAffiliation : origAffiliations) {
                        ...
                        if (origAffiliation.getId() != null && StringUtils.equals(origAffiliation.getId(),
                                entityAffiliation.getId())) {
                    ...
                    for (PersonDocumentEmploymentInfo empInfo : affiliation.getEmpInfos()) {
                        ...
                        if (empInfo.getEmploymentStatus() != null) {
                            ...
                            if (StringUtils.isNotEmpty(empInfo.getEmploymentStatusCode())) {
                                entityEmpInfo.setEmployeeStatus(EntityEmploymentStatusBo.from(
                                        getIdentityService().getEmploymentStatus(empInfo.getEmploymentStatusCode())));
                            }
                        }
                        ...
                        if (empInfo.getEmploymentType() != null) {
                            ...
                            if (StringUtils.isNotEmpty(empInfo.getEmploymentTypeCode())) {
                                entityEmpInfo.setEmployeeType(EntityEmploymentTypeBo.from(
                                        getIdentityService().getEmploymentType(empInfo.getEmploymentTypeCode())));
                            }
                        }
                        ...
                        if (ObjectUtils.isNotNull(origEmpInfos)) {
                            for (EntityEmploymentBo origEmpInfo : origEmpInfos) {
                                ...
                                if (origEmpInfo.getId() != null && StringUtils.equals(origEmpInfo.getId(),
                                        entityEmpInfo.getId())) {
    ...
    private boolean isBlankRoleQualifierVisible(String namespaceCode) {
        ...
        Parameter param = getParameterService().getParameter(namespaceCode, KRADConstants.DetailTypes.ALL_DETAIL_TYPE,
    ...
    private boolean isSameAffiliation(EntityAffiliationBo origAffiliation, EntityAffiliationBo entityAffiliation) {
        ...
        //affiliationTypeCode
        //campusCode
        return (origAffiliation != null && entityAffiliation != null) && (StringUtils.isNotEmpty(
                origAffiliation.getCampusCode()) && StringUtils.equals(origAffiliation.getCampusCode(),
                entityAffiliation.getCampusCode())) && (StringUtils.isNotEmpty(origAffiliation.getAffiliationTypeCode())
                && StringUtils.equals(origAffiliation.getAffiliationTypeCode(),
                entityAffiliation.getAffiliationTypeCode())) && (StringUtils.isNotEmpty(origAffiliation.getEntityId())
                && StringUtils.equals(origAffiliation.getEntityId(), entityAffiliation.getEntityId()));
    }
    ...
    private boolean isSameEmpInfo(EntityEmploymentBo origEmpInfo, EntityEmploymentBo entityEmpInfo) {
        ...
        return (origEmpInfo != null && entityEmpInfo != null) && (StringUtils.isNotEmpty(
                origEmpInfo.getEmploymentRecordId()) && StringUtils.equals(origEmpInfo.getEmploymentRecordId(),
                entityEmpInfo.getEmploymentRecordId())) && StringUtils.equals(origEmpInfo.getEntityId(),
                entityEmpInfo.getEntityId());
    }

    protected void setupPhone(IdentityManagementPersonDocument identityManagementPersonDocument,
            EntityTypeContactInfoBo entityType, List<EntityPhoneBo> origPhones) {
        if (!identityManagementPersonDocument.getPrivacy().isSuppressPhone() || canOverrideEntityPrivacyPreferences(
                getInitiatorPrincipalId(identityManagementPersonDocument),
                identityManagementPersonDocument.getPrincipalId())) {
                ...
                for (PersonDocumentPhone phone : identityManagementPersonDocument.getPhones()) {
                    ...
                    if (phone.getPhoneType() != null) {
                        ...
                        if (StringUtils.isNotEmpty(phone.getPhoneTypeCode())) {
                            entityPhone.setPhoneType(EntityPhoneTypeBo.from(getIdentityService().getAddressType(
                                    phone.getPhoneTypeCode())));
                        }
                    }
                    ...
                    if (ObjectUtils.isNotNull(origPhones)) {
                        for (EntityPhoneContract origPhone : origPhones) {
                            if (origPhone.getId() != null && StringUtils.equals(origPhone.getId(),
                                    entityPhone.getId())) {
    ...
    protected List<PersonDocumentPhone> loadPhones(IdentityManagementPersonDocument identityManagementPersonDocument,
            String principalId, List<EntityPhone> entityPhones, boolean suppressDisplay) {
    ...
    protected void setupEmail(IdentityManagementPersonDocument identityManagementPersonDocument,
        ...
        if (!identityManagementPersonDocument.getPrivacy().isSuppressEmail() || canOverrideEntityPrivacyPreferences(
                getInitiatorPrincipalId(identityManagementPersonDocument),
                identityManagementPersonDocument.getPrincipalId())) {
                ...
                for (PersonDocumentEmail email : identityManagementPersonDocument.getEmails()) {
                    ...
                    if (email.getEmailType() != null) {
                        ...
                        if (StringUtils.isNotEmpty(email.getEmailTypeCode())) {
                            entityEmail.setEmailType(EntityEmailTypeBo.from(getIdentityService().getEmailType(
                                    email.getEmailTypeCode())));
                        }
                    }
                    ...
                    if (ObjectUtils.isNotNull(origEmails)) {
                        for (EntityEmailContract origEmail : origEmails) {
                            if (origEmail.getId() != null && StringUtils.equals(origEmail.getId(),
                                    entityEmail.getId())) {
    ...
    protected List<PersonDocumentEmail> loadEmails(IdentityManagementPersonDocument identityManagementPersonDocument,
            String principalId, List<EntityEmail> entityEmails, boolean suppressDisplay) {
    ...
    protected void setupAddress(IdentityManagementPersonDocument identityManagementPersonDocument,
        ...
        if (!identityManagementPersonDocument.getPrivacy().isSuppressAddress() || canOverrideEntityPrivacyPreferences(
                getInitiatorPrincipalId(identityManagementPersonDocument),
                identityManagementPersonDocument.getPrincipalId())) {
                ...
                for (PersonDocumentAddress address : identityManagementPersonDocument.getAddrs()) {
                    ...
                    if (address.getAddressType() != null) {
                        ...
                        if (StringUtils.isNotEmpty(address.getAddressTypeCode())) {
                            entityAddress.setAddressType(EntityAddressTypeBo.from(getIdentityService().getAddressType(
                    ...
                    if (ObjectUtils.isNotNull(origAddresses)) {
                        for (EntityAddressContract origAddress : origAddresses) {
                            if (origAddress.getId() != null && StringUtils.equals(origAddress.getId(),
                                    entityAddress.getId())) {
    ...
    protected List<PersonDocumentAddress> loadAddresses(
            IdentityManagementPersonDocument identityManagementPersonDocument, String principalId,
            List<EntityAddress> entityAddresses, boolean suppressDisplay) {
    ...
    protected List<GroupMemberBo> populateGroupMembers(
        ...
        if (CollectionUtils.isNotEmpty(identityManagementPersonDocument.getGroups())) {
            for (PersonDocumentGroup group : identityManagementPersonDocument.getGroups()) {
                GroupMember.Builder groupPrincipalImpl = GroupMember.Builder.create(group.getGroupId(),
                        identityManagementPersonDocument.getPrincipalId(), KimGroupMemberTypes.PRINCIPAL_MEMBER_TYPE);
                ...
                Collection<GroupMember> currGroupMembers = getGroupService().getMembers(Collections.singletonList(
                        group.getGroupId()));
                if (ObjectUtils.isNotNull(currGroupMembers)) {
                    for (GroupMember origGroupMember : currGroupMembers) {
                        if (origGroupMember.isActive(new DateTime(System.currentTimeMillis())) && origGroupMember
                                .getTypeCode().equals(KimGroupMemberTypes.PRINCIPAL_MEMBER_TYPE)) {
                            if (origGroupMember.getId() != null && StringUtils.equals(origGroupMember.getId(),
                                    group.getGroupMemberId())) {
    ...
    protected List<RoleMemberBo> populateRoleMembers(
            IdentityManagementPersonDocument identityManagementPersonDocument) {
                ...
                if (role.getRolePrncpls().isEmpty()) {
                    ...
                    for (KimDocumentRoleMember roleMember : role.getRolePrncpls()) {
                        ...
                        if (roleMember.getActiveFromDate() != null) {
                            roleMemberImpl.setActiveFromDateValue(new java.sql.Timestamp(
                                    roleMember.getActiveFromDate().getTime()));
                        }
                        if (roleMember.getActiveToDate() != null) {
                            roleMemberImpl.setActiveToDateValue(new java.sql.Timestamp(
                                    roleMember.getActiveToDate().getTime()));
                        }
                        ...
                        if (ObjectUtils.isNotNull(origRoleMembers)) {
                            for (RoleMemberBo origMember : origRoleMembers) {
                                if (origMember.getRoleMemberId() != null && StringUtils.equals(
                                        origMember.getRoleMemberId(), roleMember.getRoleMemberId())) {
                            ...
                            for (KimDocumentRoleQualifier qualifier : roleMember.getQualifiers()) {
                                ...
                                if (ObjectUtils.isNotNull(origAttributes)) {
                                    for (RoleMemberAttributeDataBo origAttribute : origAttributes) {
                                        if (origAttribute.getId() != null && StringUtils.equals(origAttribute.getId(),
                                                qualifier.getAttrDataId())) {
                                            attribute.setVersionNumber(origAttribute.getVersionNumber());
                                        }
                                    }
                                }
                                if (attribute.getVersionNumber() != null || StringUtils.isNotBlank(
    ...
    protected List<DelegateBo> populateDelegations(IdentityManagementPersonDocument identityManagementPersonDocument) {
        List<DelegateBo> origDelegations = getPersonDelegations(identityManagementPersonDocument.getPrincipalId());
        List<DelegateBo> kimDelegations = new ArrayList<DelegateBo>();
        DelegateBo newKimDelegation;
        DelegateBo origDelegationImplTemp = null;
        ...
        if (CollectionUtils.isNotEmpty(identityManagementPersonDocument.getDelegations())) {
            for (RoleDocumentDelegation roleDocumentDelegation : identityManagementPersonDocument.getDelegations()) {
                newKimDelegation = new DelegateBo();
                ...
                if (ObjectUtils.isNotNull(origDelegations)) {
                    for (DelegateBo origDelegationImpl : origDelegations) {
                        if ((origDelegationImpl.getRoleId() != null && StringUtils.equals(
                                origDelegationImpl.getRoleId(), newKimDelegation.getRoleId())) && (origDelegationImpl
                                .getDelegationId() != null && StringUtils.equals(origDelegationImpl.getDelegationId(),
                                newKimDelegation.getDelegationId()))) {
                        ...
                        if (origDelegationImpl.getDelegationId() != null && StringUtils.equals(
                                origDelegationImpl.getDelegationId(), newKimDelegation.getDelegationId())) {
                ...
                newKimDelegation.setMembers(getDelegationMembers(roleDocumentDelegation.getMembers(), origMembers,
    ...
    protected List<RoleResponsibilityActionBo> populateRoleRspActions(
            IdentityManagementPersonDocument identityManagementPersonDocument) {
    ...
    public void loadRoleDoc(IdentityManagementRoleDocument identityManagementRoleDocument, Role role) {
        ...
        identityManagementRoleDocument.setDelegations(loadRoleDocumentDelegations(identityManagementRoleDocument,
                getRoleDelegations(roleBo.getId())));
        ...
        if (ObjectUtils.isNotNull(members)) {
            for (RoleMemberBo member : members) {
                ...
                if (pndMember.isActive()) {
                    ...
                    pndMember.setMemberNamespaceCode(getMemberNamespaceCode(member.getMemberTypeCode(),
                            member.getMemberId()));
    ...
    protected void loadMemberRoleRspActions(IdentityManagementRoleDocument identityManagementRoleDocument) {
        if (CollectionUtils.isNotEmpty(identityManagementRoleDocument.getMembers())) {
            for (KimDocumentRoleMember member : identityManagementRoleDocument.getMembers()) {
                member.getRoleRspActions().addAll(loadKimDocumentRoleRespActions(getRoleMemberResponsibilityActionImpls(
    ...
    protected List<KimDocumentRoleResponsibilityAction> loadKimDocumentRoleRespActions(
        ...
        List<KimDocumentRoleResponsibilityAction> documentRoleRespActions =
                new ArrayList<KimDocumentRoleResponsibilityAction>();
        ...
        if (ObjectUtils.isNotNull(roleRespActionImpls)) {
            for (RoleResponsibilityActionBo roleRespActionImpl : roleRespActionImpls) {
                ...
                if (ObjectUtils.isNotNull(roleRespActionImpl.getRoleResponsibility())) {
                    documentRoleRespAction.setKimResponsibility(
                            roleRespActionImpl.getRoleResponsibility().getKimResponsibility());
                }
    ...
    public String getMemberName(String memberTypeCode, String memberId) {
        if (StringUtils.isEmpty(memberTypeCode) || StringUtils.isEmpty(memberId)) {
            return "";
        }
    ...
    protected List<KimDocumentRoleQualifier> loadRoleMemberQualifiers(
        ...
        if (origAttributes != null) {
            for (String key : origAttributes.keySet()) {
                ...
                if (attributeDataList != null) {
                    for (RoleMemberAttributeDataBo memberRoleQualifier : attributeDataList) {
                        if (origAttributeId != null && StringUtils.equals(origAttributeId,
                                memberRoleQualifier.getKimAttribute().getId())) {
                            attributePresent = true;
                            break;
                        }
                    }
    ...
    public List<DelegateBo> getRoleDelegations(String roleId) {
        ...
        return (List<DelegateBo>) getBusinessObjectService().findMatching(DelegateBo.class, criteria);
    }

    protected List<RoleDocumentDelegation> loadRoleDocumentDelegations(
            IdentityManagementRoleDocument identityManagementRoleDocument, List<DelegateBo> delegations) {
        List<RoleDocumentDelegation> delList = new ArrayList<RoleDocumentDelegation>();
        RoleDocumentDelegation documentDelegation;
        if (ObjectUtils.isNotNull(delegations)) {
            for (DelegateBo del : delegations) {
                documentDelegation = new RoleDocumentDelegation();
                documentDelegation.setActive(del.isActive());
    ...
    protected List<RoleDocumentDelegationMember> loadDelegationMembers(
            IdentityManagementRoleDocument identityManagementRoleDocument, List<DelegateMemberBo> members) {
        List<RoleDocumentDelegationMember> pndMembers = new ArrayList<RoleDocumentDelegationMember>();
        RoleDocumentDelegationMember pndMember;
        RoleMemberBo roleMember;
        if (ObjectUtils.isNotNull(members)) {
    ...
    public void saveRole(IdentityManagementRoleDocument identityManagementRoleDocument) {
        ...
        List<RoleMemberBo> origRoleMembers = new ArrayList<RoleMemberBo>();
        List<DelegateBo> origRoleDelegations = new ArrayList<DelegateBo>();
        ...
        if (origRole == null) {
            ...
            origRolePermissions = new ArrayList<RolePermissionBo>(getBusinessObjectService().findMatching(
                    RolePermissionBo.class, altCriteria));
            origRoleResponsibilities = (List<RoleResponsibilityBo>) getBusinessObjectService().findMatching(
                    RoleResponsibilityBo.class, altCriteria);
            origRoleMembers = (List<RoleMemberBo>) getBusinessObjectService().findMatching(RoleMemberBo.class,
                    altCriteria);
            origRoleDelegations = (List<DelegateBo>) getBusinessObjectService().findMatching(DelegateBo.class,
                    altCriteria);
        }
        ...
        IdentityManagementNotificationService service =
                (IdentityManagementNotificationService) KsbApiServiceLocator.getMessageHelper()
                        .getServiceAsynchronously(new QName("KIM", "kimIdentityManagementNotificationService"));
        service.roleUpdated();
        org.kuali.rice.kim.impl.services.KIMServiceLocatorInternal.getResponsibilityInternalService()
                .updateActionRequestsForResponsibilityChange(getChangedRoleResponsibilityIds(
                        identityManagementRoleDocument, origRoleResponsibilities));
        if (!roleBo.isActive()) {
            ...
            for (KimDocumentRolePermission documentRolePermission : identityManagementRoleDocument.getPermissions()) {
                ...
                if (ObjectUtils.isNotNull(origRolePermissions)) {
                    for (RolePermissionBo origPermissionImpl : origRolePermissions) {
                        ...
                        if (origPermissionImpl.getId() != null && StringUtils.equals(origPermissionImpl.getId(),
                                newRolePermission.getId())) {
    ...
    protected List<RoleResponsibilityBo> getRoleResponsibilities(
        ...
        if (CollectionUtils.isNotEmpty(identityManagementRoleDocument.getResponsibilities())) {
            for (KimDocumentRoleResponsibility documentRoleResponsibility : identityManagementRoleDocument
                    .getResponsibilities()) {
                ...
                if (ObjectUtils.isNotNull(origRoleResponsibilities)) {
                    for (RoleResponsibilityBo origResponsibilityImpl : origRoleResponsibilities) {
                        if (!StringUtils.equals(origResponsibilityImpl.getRoleId(), newRoleResponsibility.getRoleId())
                                ...
                                && !origResponsibilityImpl.isActive()
                                && newRoleResponsibility.isActive()) {
                            newRoleResponsibility.setRoleResponsibilityId(
                                    origResponsibilityImpl.getRoleResponsibilityId());
                        }
                        if (origResponsibilityImpl.getRoleResponsibilityId() != null && StringUtils.equals(
                                origResponsibilityImpl.getRoleResponsibilityId(),
    ...
    protected List<RoleResponsibilityActionBo> getRoleResponsibilitiesActions(
        ...
        if (CollectionUtils.isNotEmpty(identityManagementRoleDocument.getResponsibilities())) {
            ...
            for (KimDocumentRoleResponsibility roleResponsibility : identityManagementRoleDocument
                    .getResponsibilities()) {
                ...
                if (!getResponsibilityInternalService().areActionsAtAssignmentLevelById(
                    ...
                    if (ObjectUtils.isNotNull(documentRoleResponsibilityActions)
                        ...
                        updateResponsibilityActionVersionNumber(roleRspAction, getRoleResponsibilityActionImpls(
                                roleResponsibility.getRoleResponsibilityId()));
    ...
    protected void updateResponsibilityActionVersionNumber(RoleResponsibilityActionBo newRoleRspAction,
        ...
        if (ObjectUtils.isNotNull(origRoleRespActionImpls)) {
            for (RoleResponsibilityActionBo origRoleResponsibilityActionImpl : origRoleRespActionImpls) {
                if (origRoleResponsibilityActionImpl.getId() != null && StringUtils.equals(
                        origRoleResponsibilityActionImpl.getId(), newRoleRspAction.getId())) {
    ...
    protected List<RoleResponsibilityActionBo> getRoleMemberResponsibilityActions(
            List<RoleMemberBo> newRoleMembersList) {
        List<RoleResponsibilityActionBo> roleRspActions = new ArrayList<RoleResponsibilityActionBo>();
    ...
    protected List<RoleMemberBo> getRoleMembers(IdentityManagementRoleDocument identityManagementRoleDocument,
            List<RoleMemberBo> origRoleMembers) {
        List<RoleMemberBo> roleMembers = new ArrayList<RoleMemberBo>();
        ...
        identityManagementRoleDocument.setKimType(KimApiServiceLocator.getKimTypeInfoService().getKimType(
                identityManagementRoleDocument.getRoleTypeId()));
        KimTypeService kimTypeService = KIMServiceLocatorWeb.getKimTypeService(
                identityManagementRoleDocument.getKimType());
            ...
            for (KimDocumentRoleMember documentRoleMember : identityManagementRoleDocument.getMembers()) {
                ...
                if (ObjectUtils.isNotNull(origRoleMembers)) {
                    for (RoleMemberBo origRoleMemberImpl : origRoleMembers) {
                        if ((origRoleMemberImpl.getRoleId() != null && StringUtils.equals(
                                origRoleMemberImpl.getRoleId(), newRoleMember.getRoleId()))
                                && (origRoleMemberImpl.getMemberId() != null && StringUtils.equals(
                                origRoleMemberImpl.getMemberId(), newRoleMember.getMemberId()))
                                && (origRoleMemberImpl.getMemberTypeCode() != null && StringUtils.equals(
                                origRoleMemberImpl.getMemberTypeCode(), newRoleMember.getMemberTypeCode()))
                                && !origRoleMemberImpl.isActive(new Timestamp(System.currentTimeMillis()))
                                && !kimTypeService.validateAttributesAgainstExisting(
                        ...
                        if (origRoleMemberImpl.getRoleMemberId() != null && StringUtils.equals(
                                origRoleMemberImpl.getRoleMemberId(), newRoleMember.getRoleMemberId())) {
                ...
                newRoleMember.setAttributeDetails(getRoleMemberAttributeData(documentRoleMember.getQualifiers(),
                        origAttributes, activatingInactive, newRoleMemberIdAssigned));
                newRoleMember.setRoleRspActions(getRoleMemberResponsibilityActions(documentRoleMember,
                        origRoleMemberImplTemp, activatingInactive, newRoleMemberIdAssigned));
    ...
    protected List<RoleResponsibilityActionBo> getRoleMemberResponsibilityActions(
            KimDocumentRoleMember documentRoleMember, RoleMemberBo origRoleMemberImplTemp, boolean activatingInactive,
            String newRoleMemberIdAssigned) {
        List<RoleResponsibilityActionBo> roleRspActions = new ArrayList<RoleResponsibilityActionBo>();
            ...
            for (KimDocumentRoleResponsibilityAction roleRspAction : documentRoleMember.getRoleRspActions()) {
                ...
                if (ObjectUtils.isNotNull(origActions)) {
                    for (RoleResponsibilityActionBo origRspAction : origActions) {
                        if (activatingInactive && StringUtils.equals(origRspAction.getRoleResponsibilityId(),
                                newRoleRspAction.getRoleResponsibilityId()) && StringUtils.equals(
                                newRoleRspAction.getRoleMemberId(), newRoleMemberIdAssigned)) {
                            newRoleRspAction.setRoleMemberId(origRspAction.getRoleMemberId());
                            newRoleRspAction.setId(origRspAction.getId());
                        }
                        if (origRspAction.getId() != null && StringUtils.equals(origRspAction.getId(),
    ...
    protected List<RoleMemberAttributeDataBo> getRoleMemberAttributeData(List<KimDocumentRoleQualifier> qualifiers,
            List<RoleMemberAttributeDataBo> origAttributes, boolean activatingInactive,
            String newRoleMemberIdAssigned) {
        ...
        if (CollectionUtils.isNotEmpty(qualifiers)) {
            for (KimDocumentRoleQualifier memberRoleQualifier : qualifiers) {
                if (StringUtils.isNotBlank(memberRoleQualifier.getAttrVal())) {
                    ...
                    if (ObjectUtils.isNotNull(origAttributes)) {
                        for (RoleMemberAttributeDataBo origAttribute : origAttributes) {
                            if (activatingInactive && StringUtils.equals(origAttribute.getKimAttributeId(),
                                    newRoleMemberAttributeData.getKimAttributeId()) && StringUtils.equals(
                                    newRoleMemberAttributeData.getAssignedToId(), newRoleMemberIdAssigned)) {
                            ...
                            if (origAttribute.getId() != null && StringUtils.equals(origAttribute.getId(),
                                    newRoleMemberAttributeData.getId())) {
    ...
    /**
     * Determines if the attribute value on the attribute data should be updated; if so, it performs some attribute value
     * formatting.
    ...
    protected AttributeDefinition getAttributeDefinition(String kimTypId, String attrDefnId) {
        ...
        if (type != null) {
            final KimTypeService typeService = (KimTypeService) KIMServiceLocatorInternal.getBean(
                    type.getServiceName());
            if (typeService != null) {
    ...
    protected List<DelegateBo> getRoleDelegations(IdentityManagementRoleDocument identityManagementRoleDocument,
            List<DelegateBo> origDelegations) {
        List<DelegateBo> kimDelegations = new ArrayList<DelegateBo>();
        DelegateBo newKimDelegation;
        DelegateBo origDelegationImplTemp = null;
        List<DelegateMemberBo> origMembers;
                ...
                if (ObjectUtils.isNotNull(origDelegations)) {
                    for (DelegateBo origDelegationImpl : origDelegations) {
                        if (StringUtils.equals(origDelegationImpl.getRoleId(), newKimDelegation.getRoleId())
                                && StringUtils.equals(origDelegationImpl.getDelegationId(),
                                newKimDelegation.getDelegationId())) {
                            //TODO: verify if you want to add  && newRoleMember.isActive() condition to if...
                            newDelegationIdAssigned = newKimDelegation.getDelegationId();
                            newKimDelegation.setDelegationId(origDelegationImpl.getDelegationId());
    ...
    protected List<DelegateMemberBo> getDelegationMembers(List<RoleDocumentDelegationMember> delegationMembers,
        ...
        if (CollectionUtils.isNotEmpty(delegationMembers)) {
            for (RoleDocumentDelegationMember delegationMember : delegationMembers) {
                ...
                if (ObjectUtils.isNotNull(origDelegationMembers)) {
                    for (DelegateMemberBo origDelegationMember : origDelegationMembers) {
                        if (activatingInactive
                                && StringUtils.equals(origDelegationMember.getMemberId(), newDelegationMemberImpl.getMemberId())
                ...
                origAttributes =
                        (origDelegationMemberImplTemp == null || origDelegationMemberImplTemp.getAttributes() == null) ?
                                new ArrayList<DelegateMemberAttributeDataBo>() :
                                origDelegationMemberImplTemp.getAttributes();
                newDelegationMemberImpl.setAttributes(getDelegationMemberAttributeData(delegationMember.getQualifiers(),
                        origAttributes, activatingInactive, delegationMemberId));
    ...
    protected List<DelegateMemberAttributeDataBo> getDelegationMemberAttributeData(
        ...
        List<DelegateMemberAttributeDataBo> delegationMemberAttributeDataList =
                new ArrayList<DelegateMemberAttributeDataBo>();
                ...
                if (StringUtils.isNotBlank(memberRoleQualifier.getAttrVal())) {
                    ...
                    if (ObjectUtils.isNotNull(origAttributes)) {
                        for (DelegateMemberAttributeDataBo origAttribute : origAttributes) {
                            if (activatingInactive && StringUtils.equals(origAttribute.getKimAttributeId(),
                                    newDelegationMemberAttributeData.getKimAttributeId()) && StringUtils.equals(
                                    newDelegationMemberAttributeData.getAssignedToId(), delegationMemberId)) {
    ...
    public void loadGroupDoc(IdentityManagementGroupDocument identityManagementGroupDocument, Group groupInfo) {
        ...
        List<GroupMember> members = new ArrayList(KimApiServiceLocator.getGroupService().getMembersOfGroup(
                groupInfo.getId()));
    ...
    protected List<GroupDocumentMember> loadGroupMembers(
        ...
        if (ObjectUtils.isNotNull(members)) {
            for (GroupMember member : members) {
                ...
                pndMember.setActiveFromDate(member.getActiveFromDate() == null ? null : new Timestamp(
                        member.getActiveFromDate().getMillis()));
                pndMember.setActiveToDate(member.getActiveToDate() == null ? null : new Timestamp(
                        member.getActiveToDate().getMillis()));
    ...
    protected List<GroupDocumentQualifier> loadGroupQualifiers(
            IdentityManagementGroupDocument IdentityManagementGroupDocument, Map<String, String> attributes) {
        ...
        if (origAttributes != null) {
            ...
            for (String key : origAttributes.keySet()) {
                ...
                if (!attributes.isEmpty()) {
                    ...
                    for (GroupAttributeBo groupQualifier : KimAttributeDataBo.createFrom(GroupAttributeBo.class,
                            attributes, IdentityManagementGroupDocument.getGroupTypeId())) {
                        if (origAttributeId != null
                                && ObjectUtils.isNotNull(groupQualifier.getKimAttribute())
    ...
    public void saveGroup(IdentityManagementGroupDocument identityManagementGroupDocument) {
        ...
        if (ObjectUtils.isNull(origGroup)) {
            ...
            origGroupMembers = (List<GroupMemberBo>) getBusinessObjectService().findMatching(GroupMemberBo.class,
                    criteria);
        }
        ...
        org.kuali.rice.kim.service.KIMServiceLocatorInternal.getGroupInternalService().updateForWorkgroupChange(
                kimGroup.getId(), oldIds, newIds);
        IdentityManagementNotificationService service =
                (IdentityManagementNotificationService) KsbApiServiceLocator.getMessageHelper()
                        .getServiceAsynchronously(new QName("KIM", "kimIdentityManagementNotificationService"));
    ...
    protected List<GroupAttributeBo> getGroupAttributeData(
            IdentityManagementGroupDocument identityManagementGroupDocument, List<GroupAttributeBo> origAttributes) {
                    ...
                    if (ObjectUtils.isNotNull(origAttributes)) {
                        for (GroupAttributeBo origAttribute : origAttributes) {
                            if (StringUtils.equals(origAttribute.getKimAttributeId(),
                                    newGroupAttributeData.getKimAttributeId()) && StringUtils.equals(
                                    newGroupAttributeData.getAssignedToId(), origAttribute.getAssignedToId())) {
                                newGroupAttributeData.setId(origAttribute.getId());
                            }
                            if (origAttribute.getId() != null && StringUtils.equals(origAttribute.getId(),
                                    newGroupAttributeData.getId())) {
    ...
    public KimDocumentRoleMember getKimDocumentRoleMember(String memberTypeCode, String memberId, String roleId) {
        ...
        List<RoleMemberBo> matchingRoleMembers = (List<RoleMemberBo>) getBusinessObjectService().findMatching(
                RoleMemberBo.class, criteria);
        if (matchingRoleMembers == null || matchingRoleMembers.size() < 1) {
            return null;
        }
    ...
    public List<KimDocumentRoleMember> getRoleMembers(Map<String, String> fieldValues) {
        ...
        if (CollectionUtils.isNotEmpty(matchingRoleMembersTemp)) {
            for (RoleMember roleMember : matchingRoleMembersTemp) {
                ...
                roleMemberObject = getMember(roleMemberBo.getMemberTypeCode(), roleMemberBo.getMemberId());
                matchingRoleMember = new KimDocumentRoleMember();
                KimCommonUtilsInternal.copyProperties(matchingRoleMember, roleMemberBo);
                matchingRoleMember.setMemberName(getMemberName(roleMemberBo.getMemberTypeCode(), roleMemberObject));
                matchingRoleMember.setMemberNamespaceCode(getMemberNamespaceCode(roleMemberBo.getMemberTypeCode(),
                        roleMemberObject));
