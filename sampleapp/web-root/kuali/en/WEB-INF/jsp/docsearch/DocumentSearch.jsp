...
<table width="100%" border=0 cellspacing=0 cellpadding=0>
  ...
  <bean:define id="documentIdLabel">
 	<bean-el:message key="docSearch.DocumentSearch.results.label.documentId"/>
  </bean:define>
  <bean:define id="typeLabel">
 	<bean-el:message key="docSearch.DocumentSearch.results.label.type"/>
  </bean:define>
  <bean:define id="titleLabel">
 	<bean-el:message key="docSearch.DocumentSearch.results.label.title"/>
  </bean:define>
  <bean:define id="routeStatusLabel">
 	<bean-el:message key="docSearch.DocumentSearch.results.label.routeStatus"/>
  </bean:define>
  ...
  <display-el:table excludedParams="*" class="bord-r-t" style="width:100%" cellspacing="0" cellpadding="0" name="${reqSearchResults}" export="true" id="result" requestURI="DocumentSearch.do?methodToCall=viewResults&key=${key}&superUserSearch=${DocumentSearchForm.superUserSearch}&isAdvancedSearch=${DocumentSearchForm.isAdvancedSearch}" pagesize="${preferences.pageSize}" defaultsort="1" defaultorder="descending"
    ...
    <display-el:column class="datacell" sortable="true" title="${documentIdLabel}" sortProperty="routeHeaderId">
       <c:choose>
    ...
    <display-el:column class="datacell" sortable="true" title="${typeLabel}" ><c:out value="${result.docTypeLabel}"/>&nbsp;</display-el:column>
    <display-el:column class="datacell" sortable="true" title="${titleLabel}" ><c:out value="${result.documentTitle}"/>&nbsp;</display-el:column>
    <display-el:column class="datacell" sortable="true" title="${routeStatusLabel}" ><c:out value="${result.docRouteStatusCodeDesc}"/>&nbsp;</display-el:column>
    <display-el:column class="datacell" sortable="true" title="${initiatorLabel}" sortProperty="initiatorTransposedName">
    ...
    <display-el:column class="datacell" sortable="true" title="${dateCreatedLabel}" sortProperty="dateCreated.time"><fmt:formatDate value="${result.dateCreated}" pattern="${Constants.DEFAULT_DATE_FORMAT_PATTERN}" />&nbsp;</display-el:column>
    ...
    <display-el:column class="datacell" style="text-align:center" title="${routeLogLabel}">
