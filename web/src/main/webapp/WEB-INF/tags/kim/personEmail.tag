...
<c:set var="canModifyEmails" scope="request" value="${!KualiForm.document.privacy.suppressEmail || KualiForm.canOverrideEntityPrivacyPreferences}" />

<kul:subtab lookedUpCollectionName="email" width="${tableWidth}" subTabTitle="Email Addresses" noShowHideButton="true">      
  	<table cellpadding="0" cellspacing="0" summary="">
		<c:choose>
			<c:when test="${!canModifyEmails}">
				<tr>
					<td>
						<bean-el:message key="warning.not.allowed.view.person.email" />
					</td>
				</tr>
			</c:when>
			<c:otherwise>
