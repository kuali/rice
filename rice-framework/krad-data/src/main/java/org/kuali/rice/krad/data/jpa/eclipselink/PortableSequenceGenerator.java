 ...
 * {@link javax.persistence.GeneratedValue} annotation. A portable sequence generator may be specified on the entity
 * class or on the primary key field or property. The scope of the generator name is global to the persistence unit
 * (across all generator types).
 *
 * <p>The term "portable" in this case indicates that native sequences will be used if the target database platform
 * supports them. However, if it does not have native sequence support, then sequence-like behavior will be emulated
 * using an appropriate strategy for the target platform.</p>
 *
 * <pre>
 *   Examples:
 *
...
public @interface PortableSequenceGenerator {
     ...
     * sequence in the schema. If the sequence construct already exists in the database or schema generation is not
     * enabled, then this value is effectively ignored.
     */
