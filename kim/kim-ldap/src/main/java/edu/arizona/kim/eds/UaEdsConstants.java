...
/**
 * This class contains constants for EDS keys -- keys into the EDS schema and
 * keys into KFS params. The keys are populated via spring from KIMLdapSpringBeans.xml
 * and spring-ldap.xml in the kfs project
 */
public class UaEdsConstants {
	 ...
	 * 
	 * Current param is: EDS_ORDERED_ACTIVE_STATUS_INDICATORS=A;L;P;W
	 * 
	 ...
	 * earns the user a 'Professional' designation, and any derived roles along
	 * with it.
	 * 
	 * Current value is:
	...
	/**
	 * The KFS EDS_RESTRICTED_EMPLOYEE_TYPES param key. Any employeeType in this
	 * param will not be granted access to KFS.
	 * 
	 * Current value is: EDS_RESTRICTED_EMPLOYEE_TYPES=J;U
	 * 
	...
	/**
	 * This is the employeeType that will be set when an EDS record returns an
	 * employeeType that is not contained in the KFS param 'EDS_EMPLOYEE_TYPES'.
	 * This is done in order to gracefully handle the cases when HR starts using
	 * new employee type codes without telling anyone (otherwise jsp's blow up
	 * when looking in KFS tables for the 'plain-english' name of the code).
	 * 
	 * IMPORTANT: As of release 58, this param is named
	 * 'EDS_DEFAULT_EMPLOYEE_TYPE', and its value is 'U'. This is important as
	 * this code is also in the 'EDS_RESTRICED_EMPLOYEE_TYPES' param, meaning
	 * the user will be restricted from logging in. This is done on purpose, as
	 * doing so will alert us to the fact that a new employeeType code exists,
	 * and we should add it to the list found under the 'EDS_EMPLOYEE_TYPES'
	 * param.
	 */
