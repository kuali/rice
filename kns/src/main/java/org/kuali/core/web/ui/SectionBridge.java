...
public class SectionBridge {
 ...
 // This method is for Entity maintenance document to have two bos combining together for a single section
    
    public static final Section toSection(MaintainableSectionDefinition sd,MaintainableSectionDefinition ssd, BusinessObject o,BusinessObject so, Maintainable maintainable, Maintainable oldMaintainable, String maintenanceAction, boolean autoFillDefaultValues, boolean autoFillBlankRequiredValues, List<String> displayedFieldNames, List<String> subDisplayedFieldNames) throws InstantiationException, IllegalAccessException {
        Section section = new Section();
        ...
        // iterate through section maint items and contruct Field UI objects
        Collection maintItems = sd.getMaintainableItems();
        Collection subMaintItems = ssd.getMaintainableItems();
        List<Row> sectionRows = new ArrayList<Row>();
        List<Field> sectionFields = new ArrayList<Field>();
        List<Field> subSectionFields = new ArrayList<Field>();
        ...
        for (Iterator iterator = subMaintItems.iterator(); iterator.hasNext();) {
            MaintainableItemDefinition item = (MaintainableItemDefinition) iterator.next();
            Field field = FieldBridge.toField(item, ssd, so, maintainable, section, autoFillDefaultValues, autoFillBlankRequiredValues, subDisplayedFieldNames);
            boolean skipAdd = false;
        ...
        if (so != null && !autoFillDefaultValues) {
            subSectionFields = FieldUtils.populateFieldsFromBusinessObject(subSectionFields, so);

            /* if maintenance action is copy, clear out secure fields */
            if (KNSConstants.MAINTENANCE_COPY_ACTION.equals(maintenanceAction)) {
                for (Iterator iterator = subSectionFields.iterator(); iterator.hasNext();) {
                    Field element = (Field) iterator.next();
                    if (element.isSecure()) {
                        element.setPropertyValue("");
                    }
        ...
        sectionRows.addAll(FieldUtils.wrapFields(sectionFields));
        sectionRows.addAll(FieldUtils.wrapFields(subSectionFields));
        section.setRows(sectionRows);
