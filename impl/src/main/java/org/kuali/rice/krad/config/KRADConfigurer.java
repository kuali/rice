...
public class KRADConfigurer extends ModuleConfigurer {
	...
	private static final String KRAD_KSB_SPRING_BEANS_PATH = "classpath:org/kuali/rice/krad/config/KRADServiceBusSpringBeans.xml";
    ...
    private void loadDataDictionary() {
		if (isLoadDataDictionary()) {
			...
			if ( dds == null ) {
				dds = (DataDictionaryService) GlobalResourceLoader.getService(KRADServiceLocatorWeb.DATA_DICTIONARY_SERVICE);
			}
   ...
   /**
    ...
    protected void configureDataSource() {
        ...
        if (getServerDataSource() != null) {
        	ConfigContext.getCurrentContextConfig().putObject(KRADConstants.KRAD_SERVER_DATASOURCE, getServerDataSource());
        }
    }
