...
<c:set var="canModifyAddresses" scope="request" value="${!KualiForm.document.privacy.suppressAddress || KualiForm.canOverrideEntityPrivacyPreferences}" />

<kul:subtab lookedUpCollectionName="address" width="${tableWidth}" subTabTitle="Addresses" noShowHideButton="true">      
   	<table cellpadding="0" cellspacing="0" summary="">
		<c:choose>
			<c:when test="${!canModifyAddresses}">
				<tr>
					<td>
						<bean-el:message key="warning.not.allowed.view.person.address" />
					</td>
				</tr>
			</c:when>
			<c:otherwise>
