...
public class ReferenceLinker {
    ...
    protected void linkCollections( DataObjectMetadata metadata, Object persistableObject, Set<Object> referenceSet ) {
        ...
        for ( DataObjectCollection collectionMetadata : collections ) {
            ...
            List<DataObjectAttributeRelationship> collectionAttributeRelationships = collectionMetadata.getAttributeRelationships();

            // Need to iterate through the collection, setting FK values as needed and telling each child object to link itself
            for ( Object collectionItem : collection ) {
                ...
                // Now - go through the keys relating the parent object to each child and set them so that they are linked properly
                for ( DataObjectAttributeRelationship rel : collectionAttributeRelationships ) {
                    collItemWrapper.setPropertyValue(
                            rel.getChildAttributeName(),
