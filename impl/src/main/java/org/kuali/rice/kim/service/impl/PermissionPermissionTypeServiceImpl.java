 ...
 *
 */
public class PermissionPermissionTypeServiceImpl extends NamespacePermissionTypeServiceImpl {

	{
		inputRequiredAttributes.add(KimAttributes.PERMISSION_NAME);
	}
	...
	protected boolean performMatch(AttributeSet inputAttributeSet, AttributeSet storedAttributeSet) {
		...
		if (StringUtils.isEmpty(storedAttributeSet.get(KimAttributes.PERMISSION_NAME))) {
        	return namespaceMatch;
		}

		return namespaceMatch 
			&& inputAttributeSet.get(KimAttributes.PERMISSION_NAME).equals(storedAttributeSet.get(KimAttributes.PERMISSION_NAME));
	}
