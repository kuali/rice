        ...
        if (!(form instanceof InquiryForm)) {
            ...
            try {
            	if(!KNSConstants.DOWNLOAD_BO_ATTACHMENT_METHOD.equals(methodToCall)){	
            		...
            		if (!KIMServiceLocator.getIdentityManagementService().isAuthorizedByTemplateName(GlobalVariables.getUserSession().getPrincipalId(), KNSConstants.KNS_NAMESPACE, KimConstants.PermissionTemplateNames.INQUIRE_INTO_RECORDS, KNSUtils.getNamespaceAndComponentSimpleName(businessObjectClass), null)) {
