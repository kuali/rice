...
public class KualiPortalAction extends KualiSimpleAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (gotoUrl != null) {
            ...
            Pattern pattern = Pattern.compile(ConfigContext.getCurrentContextConfig().getProperty(KNSConstants.PORTAL_ALLOWED_REGEX));
            Matcher matcher = pattern.matcher(gotoUrl);
            if(!matcher.matches()) {
                throw new KualiException("The requested channel URL is not authorized for display in portal.");
            }
        }
