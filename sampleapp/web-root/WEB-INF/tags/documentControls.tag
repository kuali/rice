        ...
        <%--c:if test="${KualiForm.documentActionFlags.canAnnotate and not suppressRoutingControls and not KualiForm.suppressAllButtons}">
            <div class="annotate">
              <table width="100%" cellpadding="0" cellspacing="0" class="annotate-top" summary="">
                <tr>
                  <td class="annotate-t"><img src="images/annotate-tl1.gif" alt="" width="12" height="24" align="middle" class="annotate-t">
                    <label for="routing annotation">Routing Annotation:</label></td>
                  <td class="annotate-t"><div align="right"><img src="images/annotate-tr1.gif" alt="" width="12" height="24" align="middle"></div></td>
                </tr>
              </table>
		...
		<c:if test="${not KualiForm.suppressAllButtons}">
	        <div id="globalbuttons" class="globalbuttons">
	            ...
	            <c:if test="${KualiForm.documentActionFlags.canPerformRouteReport and not suppressRoutingControls}">
				    <html:image src="images/buttonsmall_routereport.gif" styleClass="globalbuttons" property="methodToCall.performRouteReport" title="Perform Route Report" alt="Perform Route Report" />
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canSupervise and not suppressRoutingControls}">
				    <html:image src="images/buttonsmall_supervfunct.gif" styleClass="globalbuttons" property="methodToCall.supervisorFunctions" title="Supervisor Functions" alt="Supervisor Functions" />
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canRoute and not suppressRoutingControls}">
	                <html:image src="images/buttonsmall_submit.gif" styleClass="globalbuttons" property="methodToCall.route" title="submit" alt="submit"/>
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canSave and not viewOnly}">
	                <html:image src="images/buttonsmall_save.gif" styleClass="globalbuttons" property="methodToCall.${saveButtonValue}" title="save" alt="save"/>
	            </c:if>
	             <c:if test="${KualiForm.documentActionFlags.canReload}">
	                <html:image src="images/buttonsmall_reload.gif" styleClass="globalbuttons" property="methodToCall.reload" title="reload" alt="reload" onclick="excludeSubmitRestriction=true"/>
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canBlanketApprove and not suppressRoutingControls}">
	                <html:image src="images/buttonsmall_blanketapp.gif" styleClass="globalbuttons" property="methodToCall.blanketApprove" title="blanket approve" alt="blanket approve"/>
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canApprove and not suppressRoutingControls}">
	                <html:image src="images/buttonsmall_approve.gif" styleClass="globalbuttons" property="methodToCall.approve" title="approve" alt="approve"/>
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canDisapprove and not suppressRoutingControls}">
	                <html:image src="images/buttonsmall_disapprove.gif" styleClass="globalbuttons" property="methodToCall.disapprove" title="disapprove" alt="disapprove"/>
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canFYI and not suppressRoutingControls}">
	                <html:image src="images/buttonsmall_fyi.gif" styleClass="globalbuttons" property="methodToCall.fyi" title="fyi" alt="fyi"/>
	            </c:if>
	            <c:if test="${KualiForm.documentActionFlags.canAcknowledge and not suppressRoutingControls}">
	                <html:image src="images/buttonsmall_acknowledge.gif" styleClass="globalbuttons" property="methodToCall.acknowledge" title="acknowledge" alt="acknowledge"/>
	            </c:if>
	            ...
	            <c:if test="${KualiForm.documentActionFlags.canCancel}">
	                <html:image src="images/buttonsmall_cancel.gif" styleClass="globalbuttons" property="methodToCall.cancel" title="cancel" alt="cancel"/>
	            </c:if>
	                <c:if test="${KualiForm.documentActionFlags.canCopy}">
                    <html:image src="images/buttonsmall_copy.gif" styleClass="globalbuttons" property="methodToCall.copy" title="Copy current document" alt="Copy current document"/>
	                </c:if>
	            <c:if test="${transactionalDocument}">
	                <c:if test="${KualiForm.documentActionFlags.canErrorCorrect}">
	                    <html:image src="images/buttonsmall_errcorr.gif" styleClass="globalbuttons" property="methodToCall.correct" title="Create error correction document from current document" alt="Create error correction document from current document"/>
	                </c:if>
	            </c:if>
	        </div>
        </c:if>
