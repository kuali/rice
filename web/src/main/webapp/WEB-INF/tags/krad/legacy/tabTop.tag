...
<!--
<input type="hidden" name="tabStates(${tabKey})" value="${(isOpen ? 'OPEN' : 'CLOSE')}" />"
<html:hidden path="tabStates(${tabKey})" value="${(isOpen ? 'OPEN' : 'CLOSE')}" />
-->
<!-- TAB -->
	...
	<tr>
		...
		<td class="tabtable1-mid">
            <c:if test="${isOpen == 'true' || isOpen == 'TRUE'}">
			    <html-el:image property="methodToCall.toggleTab.tab${tabKey}"	src="${ConfigProperties.kr.externalizable.images.url}tinybutton-hide.gif" title="close ${tabTitle}" alt="close ${tabTitle}" styleClass="tinybutton" styleId="tab-${tabKey}-imageToggle" onclick="javascript: return toggleTab(document, '${tabKey}'); " tabindex="-1" />
		    </c:if>
		    <c:if test="${isOpen != 'true' && isOpen != 'TRUE'}">
			    <html-el:image property="methodToCall.toggleTab.tab${tabKey}"	src="${ConfigProperties.kr.externalizable.images.url}tinybutton-show.gif" title="open ${tabTitle}" alt="open ${tabTitle}" styleClass="tinybutton" styleId="tab-${tabKey}-imageToggle" onclick="javascript: return toggleTab(document, '${tabKey}'); " tabindex="-1" />
		    </c:if>
		</td>
