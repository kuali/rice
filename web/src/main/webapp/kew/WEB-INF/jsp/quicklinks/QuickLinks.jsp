			...
			<td lass="datacell" valign=top>
				<ul>
					<c:forEach begin="0" end="4" var="namedSearch" items="${QuickLinksForm.namedSearches}" >
						<li><a target="_blank" title="<c:out value="${namedSearch.value}"/>" href="${ConfigProperties.kr.url}/lookup.do?methodToCall=refresh&backLocation=http://localhost:8080/kr-dev/portal.do&businessObjectClassName=org.kuali.rice.kew.docsearch.DocSearchCriteriaDTO&hideReturnLink=true&refreshCaller=customLookupAction&showMaintenanceLinks=Yes&savedSearchName=DocSearch.NamedSearch.<c:out value="${namedSearch.value}"/>&formKey=88888888"><c:out value="${namedSearch.value}"/></a>
					</c:forEach>
				</ul>
			</td>
			<td lass="datacell" valign=top>
				<ul>
					<c:forEach begin="0" end="4" var="recentSearch" items="${QuickLinksForm.recentSearches}" >
						<li><a target="_blank" title="<c:out value="${namedSearch.value}"/>" href="${ConfigProperties.kr.url}/lookup.do?methodToCall=refresh&backLocation=http://localhost:8080/kr-dev/portal.do&businessObjectClassName=org.kuali.rice.kew.docsearch.DocSearchCriteriaDTO&hideReturnLink=true&refreshCaller=customLookupAction&showMaintenanceLinks=Yes&savedSearchName=<c:out value="${recentSearch.key}"/>&formKey=88888888"><%=((org.kuali.rice.kew.web.KeyValue)pageContext.getAttribute("recentSearch")).getValue().length() > 100 ? ((org.kuali.rice.kew.web.KeyValue)pageContext.getAttribute("recentSearch")).getValue().substring(0,100) + "..." : ((org.kuali.rice.kew.web.KeyValue)pageContext.getAttribute("recentSearch")).getValue()%></a>
					</c:forEach>
				</ul>
			</td>
		</tr>
