...
/**
 * A generic Role Attribute superclass that can be used to route to an ID.  Can take as configuration the
...
public abstract class AbstractIdRoleAttribute extends AbstractRoleAttribute implements GenericXMLRuleAttribute {
     ...
     * 
     * @see org.kuali.rice.kew.plugin.attributes.RoleAttribute#getQualifiedRoleNames(java.lang.String, edu.iu.uis.eden.routeheader.DocumentContent)
     */
	...
	try {
	    ...
	    NodeList idNodes = (NodeList)xPath.evaluate("//" + getAttributeElementName() + "/" + elementName, documentContent.getDocument(), XPathConstants.NODESET);
     ...
     * 
     * @see edu.iu.uis.eden.routetemplate.AbstractWorkflowAttribute#getDocContent()
     */
    @Override
	...
	if (!StringUtils.isBlank(getIdValue())) {
	    String elementName = (String)getParamMap().get(XML_ELEMENT_LABEL);
	    return "<" + getAttributeElementName() + "><" + elementName + ">" + getIdValue() + "</" + elementName + "></" + getAttributeElementName() + ">";
	}
	return "";
    }
	...
	if (ruleAttribute != null) {
		...
		XPath xPath = XPathHelper.newXPath();
		try {
		    String xmlElementLabel = xPath.evaluate("/configuration/" + XML_ELEMENT_LABEL, new InputSource(new StringReader(xmlConfigData)));
		    String roleNameLabel = xPath.evaluate("/configuration/" + ROLE_NAME_LABEL, new InputSource(new StringReader(xmlConfigData)));
